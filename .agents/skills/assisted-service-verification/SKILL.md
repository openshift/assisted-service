...
```bash
skipper run go test -v ./internal/cluster -run TestClusterName    # by test name
skipper make unit-test FOCUS="install_cluster"             # Ginkgo focus + DB initialization and teardown
skipper make unit-test TEST="./internal/cluster"  # execute tests for package cluster
```
...
```
1. skipper run go mod tidy
2. skipper run go mod vendor
3. skipper run go build ./...
4. skipper make generate                  # regenerate everything
5. git diff --exit-code                   # verify no generated code diff
6. skipper make lint
7. skipper make unit-test TEST=<affected packages>
```
