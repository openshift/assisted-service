...
func readQueuesListRequest(request *QueuesListServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeQueuesListResponse(response *QueuesListServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(QueueListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeQueueList(response.items.items, stream)
			count++
		}
	}
