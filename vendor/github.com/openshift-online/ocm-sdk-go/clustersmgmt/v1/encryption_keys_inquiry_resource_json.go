...
func readEncryptionKeysInquirySearchRequest(request *EncryptionKeysInquirySearchServerRequest, r *http.Request) error {
	var err error
	query := r.URL.Query()
...
func writeEncryptionKeysInquirySearchResponse(response *EncryptionKeysInquirySearchServerResponse, w http.ResponseWriter) error {
	...
	count := 1
	stream.WriteString(EncryptionKeyListKind)
	...
	if response.items != nil {
		if response.items.items != nil {
			...
			writeEncryptionKeyList(response.items.items, stream)
			count++
		}
	}
