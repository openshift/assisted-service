...
// dispatchNodesMetricQuery navigates the servers tree rooted at the given server
// till it finds one that matches the given set of path segments, and then invokes
// the corresponding server.
func dispatchNodesMetricQuery(w http.ResponseWriter, r *http.Request, server NodesMetricQueryServer, segments []string) {
	if len(segments) == 0 {
		switch r.Method {
		case "GET":
			adaptNodesMetricQueryGetRequest(w, r, server)
			return
		default:
...
// adaptNodesMetricQueryGetRequest translates the given HTTP request into a call to
// the corresponding method of the given server. Then it translates the
// results returned by that method into an HTTP response.
func adaptNodesMetricQueryGetRequest(w http.ResponseWriter, r *http.Request, server NodesMetricQueryServer) {
	request := &NodesMetricQueryGetServerRequest{}
	err := readNodesMetricQueryGetRequest(request, r)
	if err != nil {
		glog.Errorf(
	...
	response := &NodesMetricQueryGetServerResponse{}
	response.status = 200
	...
	err = writeNodesMetricQueryGetResponse(response, w)
	if err != nil {
		glog.Errorf(
