...
var _ = Describe("Operators endpoint tests", func() {
	...
	Context("OLM operators", func() {
		...
		It("LSO as ODF dependency on ARM arch", func() {
			...
			Expect(*reason).To(ContainSubstring("cannot use Local Storage Operator because it's not compatible with the arm64 architecture on version 4.19"))
		})
		...
		It("should lvm have right subscription name on 4.19", func() {
			cluster := registerNewCluster(
				"4.19.0",
				int64(1),
				[]*models.OperatorCreateParams{{Name: cnv.Operator.Name}},
