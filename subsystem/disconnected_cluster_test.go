...
var _ = Describe("Disconnected Cluster", func() {
	...
	It("registers with Kind=DisconnectedCluster and Status=Unmonitored", func() {
		...
		Expect(err).NotTo(HaveOccurred())
		Expect(swag.StringValue(got.Payload.Status)).To(Equal(models.ClusterStatusUnmonitored))
