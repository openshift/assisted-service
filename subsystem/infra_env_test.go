...
- ipv4:
  ...
  type: ethernet`
	...
	It("ipxe with boot control", func() {
		...
		utils_test.TestContext.GenerateHWPostStepReply(context.Background(), host, getValidWorkerHwInfoWithCIDR("1.2.3.4/24"), "h1")
		...
		utils_test.TestContext.GenerateHWPostStepReply(context.Background(), host2, getValidWorkerHwInfoWithCIDR("1.2.3.5/24"), "h2")
		scriptResp, err = http.Get(url)
		Expect(err).NotTo(HaveOccurred())
