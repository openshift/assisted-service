	...
	Context("Support Level", func() {
		...
		Context("Update cluster", func() {
			It("Update umn true won't fail with multi release without infra-env", func() {
				cluster, err := registerNewCluster("4.16-multi", common.MultiCPUArchitecture, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
			...
			It("Update umn true fail with s390x with infra-env", func() {
				...
				cluster, err := registerNewCluster("4.16-multi", common.S390xCPUArchitecture, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.MultiCPUArchitecture))
			...
			Context("UpdateInfraEnv", func() {
				...
				It("Update ppc64le infra env minimal iso with cluster", func() {
					cluster, err := registerNewCluster("4.16", models.ClusterCPUArchitecturePpc64le, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
					Expect(err).NotTo(HaveOccurred())
				...
				It("Update s390x infra env minimal iso with cluster - fail", func() {
					cluster, err := registerNewCluster("4.16", "s390x", int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
					Expect(err).NotTo(HaveOccurred())
		...
		Context("Register cluster", func() {
			...
			It("Register cluster won't fail with s390x without UMN", func() {
				cluster, err := registerNewCluster("4.16", "s390x", int64(common.MinMasterHostsNeededForInstallationInHaMode), nil)
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
			})
			It("Register SNO cluster won't fail with s390x", func() {
				cluster, err := registerNewCluster("4.16", "s390x", int64(1), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
			})
			It("Register SNO cluster won't fail with s390x without UMN", func() {
				cluster, err := registerNewCluster("4.16", "s390x", int64(1), nil)
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
			})
		}) // Register cluster
