...
var _ = Describe("Feature support levels API", func() {
	...
	Context("Support Level", func() {
		...
		Context("Update cluster", func() {
			It("Update umn true won't fail on 4.19 with multi release without infra-env", func() {
				cluster, err := registerNewCluster("4.19-multi", common.MultiCPUArchitecture, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
			...
			It("Update umn true fail on 4.19 with s390x with infra-env", func() {
				expectedError := "cannot use Cluster Managed Networking because it's not compatible with the s390x architecture on version 4.19"
				cluster, err := registerNewCluster("4.19-multi", common.S390xCPUArchitecture, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.MultiCPUArchitecture))

				infraEnv, err := registerNewInfraEnv(cluster.Payload.ID, "4.19", "s390x")
				Expect(err).NotTo(HaveOccurred())
			...
			It("Create infra-env after updating OLM operators on s390x architecture ", func() {
				expectedError := "cannot use OpenShift Virtualization because it's not compatible with the s390x architecture on version 4.19"
				cluster, err := registerNewCluster("4.19-multi", common.S390xCPUArchitecture, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.MultiCPUArchitecture))
				...
				_, err = registerNewInfraEnv(cluster.Payload.ID, "4.19", "s390x")
			...
			Context("UpdateInfraEnv", func() {
				It("Update ppc64le infra env minimal iso without cluster", func() {
					infraEnv, err := registerNewInfraEnv(nil, "4.19", models.ClusterCPUArchitecturePpc64le)
					Expect(err).ToNot(HaveOccurred())
				...
				It("Update ppc64le infra env minimal iso with cluster", func() {
					cluster, err := registerNewCluster("4.19", models.ClusterCPUArchitecturePpc64le, int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
					Expect(err).NotTo(HaveOccurred())

					infraEnv, err := registerNewInfraEnv(cluster.Payload.ID, "4.19", models.ClusterCPUArchitecturePpc64le)
					Expect(err).ToNot(HaveOccurred())
					Expect(common.ImageTypeValue(infraEnv.Payload.Type)).ToNot(Equal(models.ImageTypeMinimalIso))
				...
				It("Update s390x infra env minimal iso with cluster - fail", func() {
					cluster, err := registerNewCluster("4.19", "s390x", int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
					Expect(err).NotTo(HaveOccurred())

					infraEnv, err := registerNewInfraEnv(cluster.Payload.ID, "4.19", models.ClusterCPUArchitectureS390x)
					Expect(err).ToNot(HaveOccurred())
					Expect(common.ImageTypeValue(infraEnv.Payload.Type)).ToNot(Equal(models.ImageTypeMinimalIso))
					...
					ExpectWithOffset(1, *err2.Payload.Reason).To(ContainSubstring("cannot use Minimal ISO because it's not compatible with the s390x architecture on version 4.19"))
				})
			})
		})

		Context("Register cluster", func() {
			It("Register cluster won't fail on 4.19 with s390x", func() {
				cluster, err := registerNewCluster("4.19", "s390x", int64(common.MinMasterHostsNeededForInstallationInHaMode), swag.Bool(true))
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
			})

			It("Register cluster won't fail on 4.19 with s390x without UMN", func() {
				cluster, err := registerNewCluster("4.19", "s390x", int64(common.MinMasterHostsNeededForInstallationInHaMode), nil)
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
			})

			It("Register SNO cluster won't fail with s390x", func() {
				cluster, err := registerNewCluster("4.19", "s390x", int64(1), nil)
				Expect(err).NotTo(HaveOccurred())
				Expect(cluster.Payload.CPUArchitecture).To(Equal(common.S390xCPUArchitecture))
