...
const (
	...
	// StepTypeTimestamp captures enum value "timestamp"
	StepTypeTimestamp StepType = "timestamp"
)
...
func init() {
	var res []StepType
	if err := json.Unmarshal([]byte(`["connectivity-check","execute","inventory","install","free-network-addresses","reset-installation","dhcp-lease-allocate","api-vip-connectivity-check","timestamp"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
