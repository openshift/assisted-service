...
type Cluster struct {
	...
	// Required: true
	// Enum: [insufficient ready error preparing-for-installation pending-for-input installing finalizing installed adding-hosts cancelled installing-pending-user-action created]
...
func init() {
	var res []string
	if err := json.Unmarshal([]byte(`["insufficient","ready","error","preparing-for-installation","pending-for-input","installing","finalizing","installed","adding-hosts","cancelled","installing-pending-user-action","created"]`), &res); err != nil {
		panic(err)
	}
	for _, v := range res {
...
const (
	...
	// ClusterStatusCreated captures enum value "created"
	ClusterStatusCreated string = "created"
)
