...
### 5.2 Explanation of Placeholder Values
...
|------------|---------|------------------|
| **`<INFRA_ID>`** | The unique infrastructure ID generated by OpenShift for your cluster. It is used to name MachineSets, Machines, and other cloud resources. | Run:<br>```oc get infrastructure cluster -o jsonpath='{.status.infrastructureName}'```<br>Example: `mycluster-abc12` |
| **`<PORTGROUP_NAME>`** | The vSphere network (portgroup) the VM should attach to. Machine API will attach the cloned VM to this network, and RHCOS networking will configure itself (DHCP or Ignition). | In vCenter: **Networking → Distributed Port Groups**<br>Examples: `VM Network`, `ci-vlan-1221`, `Prod-Network-01` |
| **`<RHCOS_VERSION>`** | The version of RHCOS you imported as an OVA template. This appears only in the template name you created. | Whatever you named the template, e.g.:<br>`rhcos-4.20.0` |
| **`DATA_CENTER`** | The vSphere Datacenter containing the cluster’s VMs, templates, datastore, and resources. | vSphere UI → Inventory browser.<br>Same value appears under:<br>`Infrastructure.spec.platformSpec.vsphere.failureDomains[*].topology.datacenter` |
| **`DATASTORE_NAME`** | The name of the datastore where new worker VMs will be created. **Must be the short name**, not the full inventory path. | vSphere UI → Datastores view.<br>Also appears in `Infrastructure` object under `.spec.platformSpec.vsphere.failureDomains[*].topology.datastore` (as a full path-use just the datastore name part) |
| **`VCENTER.EXAMPLE.COM`** | The fully-qualified domain name of your vCenter server. | URL used to log into vSphere.<br>Also under:<br>`Infrastructure.spec.platformSpec.vsphere.vcenters[*].server` |
| **`/DATA_CENTER/vm/path/to/template/...`** | The **inventory path** in vSphere to the folder containing your RHCOS template. Machine API must be able to find the template at this exact path. | Right-click the template in vSphere → **Copy Inventory Path** |
| **`/DATA_CENTER/host/CLUSTER_NAME/Resources`** | The vSphere Resource Pool in which the new VMs will run. If you use a cluster, it's usually `/DATACENTER/host/<Cluster>/Resources`. | vSphere → Hosts & Clusters → right-click your cluster → **Resource Pool** tab |
| **`/DATA_CENTER/vm/path/to/cluster/folder`** | The folder where cloned worker VMs should be placed. This controls where the VMs appear in the vCenter UI. | vSphere → Inventory browser → copy folder inventory path |
| **`<CLUSTER_NAME>`** | The name of the vSphere cluster containing your ESXi hosts. Used inside the resource pool path. | vSphere → Hosts & Clusters → left tree view |
...
On most OpenShift 4.15+ clusters, worker CSRs auto-approve by default (unless cluster admins disabled it via custom RBAC). If you see `Pending` CSRs for more than 2 minutes, approve them manually:
...
```
NAME        AGE   SIGNERNAME                                    REQUESTOR                       CONDITION
csr-abcde   40s   kubernetes.io/kube-apiserver-client-kubelet   system:serviceaccount:...       Approved,Issued
...
Check that the node appears and reaches `Ready` status:
...
```
NAME                                        STATUS   ROLES    AGE     INTERNAL-IP    EXTERNAL-IP   OS-IMAGE
my-test-cluster-d7-jzsct-master-0        Ready    master   1d      10.184.6.171   <none>        Red Hat Enterprise Linux CoreOS ...
my-test-cluster-d7-jzsct-master-1        Ready    master   1d      10.184.6.172   <none>        Red Hat Enterprise Linux CoreOS ...
my-test-cluster-d7-jzsct-master-2        Ready    master   1d      10.184.6.173   <none>        Red Hat Enterprise Linux CoreOS ...
my-test-cluster-d7-jzsct-worker-xxxxx    Ready    worker   2m      10.184.6.174   <none>        Red Hat Enterprise Linux CoreOS ...
```
...
- ✅ Status is `Ready`
- ✅ Age is recent (seconds or minutes)
- ✅ IP address is from the correct network range
...
```bash
oc scale machineset my-test-cluster-d7-jzsct-worker -n openshift-machine-api --replicas=2
```
...
- A second Machine is created and progresses through Provisioning → Provisioned → Running
- A second worker node joins and becomes `Ready`
- MachineSet shows `DESIRED: 2, CURRENT: 2, READY: 2, AVAILABLE: 2`

### 9.2 Manual scale-down

Reduce replicas back to 1:

```bash
oc scale machineset my-test-cluster-d7-jzsct-worker -n openshift-machine-api --replicas=1
```

> **Warning:** Scaling down will permanently delete the VM from vSphere. The Machine API will power off and delete the VM-it cannot be recovered.

**The Machine API will:**

1. Select a worker to remove
2. Drain pods from that node
3. Delete the Machine object
4. Power off and delete the VM from vSphere
5. Remove the node from the cluster

**Verify scale-down:**

```bash
# Watch Machine deletion
oc get machines -n openshift-machine-api -w

# Watch node removal
oc get nodes -w

# Verify in vSphere UI that the VM was deleted
```

**Expected:**

- One Machine transitions to `Deleting` phase, then disappears
- Corresponding node is removed from `oc get nodes`
- VM is removed from vSphere inventory

---

## 10. Verify autoscaling (optional)

If you configured autoscaling (Section 7), verify it works correctly.

### 10.1 Force scale-up by creating pending workloads

Create a test deployment with high resource requests to trigger autoscaling:

```bash
# Create a deployment that requests more resources than available
oc create deployment autoscale-test --image=registry.access.redhat.com/ubi8/ubi:latest -- sleep infinity
oc set resources deployment autoscale-test --requests=cpu=1,memory=2Gi
oc scale deployment autoscale-test --replicas=20
```

**Watch for autoscaling:**

```bash
# Check for pending pods
oc get pods -l app=autoscale-test
...
1. Some pods enter `Pending` state (insufficient resources)
2. ClusterAutoscaler detects pending pods
3. MachineAutoscaler increases MachineSet replicas
4. New Machines are created
5. New workers join the cluster
6. Pending pods schedule on new nodes

### 10.2 Check autoscaler logs

Monitor the cluster autoscaler to see scaling decisions:
...
```
Scale-up: group my-test-cluster-d7-jzsct-worker size set to 3
Pod autoscale-test-xxxxx is unschedulable
Increasing my-test-cluster-d7-jzsct-worker min size from 2 to 3
```

### 10.3 Test scale-down

Delete the test workload and verify autoscaler removes excess nodes:

```bash
# Delete the test deployment
oc delete deployment autoscale-test

# Watch for scale-down (default: 10 minutes after nodes become underutilized)
oc get nodes -w
oc get machines -n openshift-machine-api -w
```
...
1. Test pods are deleted
2. Nodes become underutilized
3. After `unneededTime` (default: 10m), autoscaler marks nodes for removal
4. Nodes are drained and removed
5. MachineSet scales back down to `minReplicas`
