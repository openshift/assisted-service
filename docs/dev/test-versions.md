...
`TestVersion()` in `internal/common` resolves OCP versions dynamically from the data files (`data/default_release_images.json`). Available versions are auto-generated into `internal/common/test_versions_generated.go` as part of the standard `make generate` target.
...
| `.ReleaseVersion()` | `string` | Full release version (with patch), e.g. `"4.22.0"` |
| `.ReleaseImageURL()` | `string` | Full quay.io release image URL |
| `.RhcosVersion()` | `string` | RHCOS version string |
| `.RhcosImageURL()` | `string` | Full RHCOS image URL |
| `.MultiVersion()` | `string` | Version with `-multi` suffix for multi-arch DB fields |
| `.MultiReleaseVersion()` | `string` | ReleaseVersion with `-multi` suffix |
...
When OCP versions are added or removed from `data/default_release_images.json`, run:

```bash
make generate
```

This regenerates `internal/common/test_versions_generated.go` with the current version-to-architecture mapping.
