...
- Is set automatically when a `disconnected-iso` type InfraEnv is registered for a cluster in `insufficient` status
- Prevents the cluster from being monitored or transitioned by the state machine
...
- This guarantees the cluster stays in a safe state until the InfraEnv updates it to `disconnected`


**Implementation Approach:**
1. **Cluster Creation**: UI creates a cluster with 0 hosts and required configuration fields
2. **Status Update**: When InfraEnv with `disconnected-iso` type is registered, cluster status is updated to `disconnected`
3. **Monitoring Exclusion**: Disconnected clusters are excluded from monitoring at the database query level
4. **State Machine Isolation**: Disconnected status prevents any further state transitions

This approach ensures temporary clusters don't consume monitoring resources or generate unnecessary events while maintaining a clear distinction between actual clusters and configuration containers.
