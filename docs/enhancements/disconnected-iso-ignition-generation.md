...
- This creates a cluster with `DisconnectedCluster` kind and `created` status
...
Introduce a new cluster type (`DisconnectedCluster`) that serves as a configuration container for ISO generation. These clusters are created via `/v2/clusters/disconnected`, start directly in `created` status bypassing the state machine, and are excluded from monitoring. When an InfraEnv with `disconnected-iso` image type is associated with such a cluster, the discovery.ign endpoint returns customized ignition generated using openshift-install's unconfigured-ignition command.
...
**New Endpoint:**
- **POST** `/v2/clusters/disconnected` - Creates a cluster with `DisconnectedCluster` kind and `created` status
  - Accepts same parameters as regular cluster registration (`cluster-create-params`)
  - Returns cluster object with kind set to `DisconnectedCluster`
  - Cluster is created directly in `created` status without state machine transitions

**Cluster Model Changes:**
- Add `DisconnectedCluster` to the `kind` enum (alongside `Cluster` and `AddHostsCluster`)
- Add `created` as a new cluster status
...
- **Status**: Created directly in `created` status, bypassing the state machine
...
- Are created with their final status (`created`) immediately
...
- The status is determined in `registerClusterInternalWithKind` based on the cluster kind:
  - `DisconnectedCluster` → `created`
  - `AddHostsCluster` → `adding-hosts`
  - Regular `Cluster` → `insufficient`

**Monitoring Exclusion:**
Clusters with `created` status are excluded from monitoring:
- The monitoring query generator filters out `created` status at the database level
