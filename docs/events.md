...
The Assisted Service supports configurable rate limiting to prevent event flooding. Rate limiting throttles high-frequency events on a per-cluster, per-host, or per-infraenv basis.

### Configuration

Configure rate limits using the `EVENT_RATE_LIMITS` environment variable with JSON format:
...
At startup, the service logs configured limits:
```
INFO Event rate limits configured  upgrade_agent_failed=2h0m0s upgrade_agent_finished=1h0m0s
```

### Monitoring

Discarded events are logged:
```
WARN Event will be discarded  name=upgrade_agent_failed limit=2h0m0s count=1
```
