...
3. **nmstate Development Headers** (for network state management tests)
   ...
   **Note:** Package installation commands may require elevated permissions or explicit approval depending on your AI coding agent's security model.
...
**When writing unit tests in `internal/` or `pkg/` packages**, reference: `.agents/skills/assisted-service-writing-unit-tests/SKILL.md`
...
**Note:** These package installation commands may require elevated permissions or explicit approval depending on your AI coding agent's security model.
...
1. Install nmstate-devel: `sudo dnf install -y nmstate-devel`
   - **Note:** This command may require elevated permissions or explicit approval depending on your AI coding agent's security model.
...
When an AI coding agent assists with commits, you MUST add AI attribution as the last line of every commit message. Either format is valid:

```
Assisted-by: AI-Agent-Name <agent-email>
```

or:

```
Co-Authored-By: AI-Agent-Name <agent-email>
```
