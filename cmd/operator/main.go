...
func main() {
	...
	localClusterImportReconciler, err := controllers.NewLocalClusterImportReconciler(mgr.GetClient(), "local-cluster", log)
	if err != nil {
		setupLog.Error(err, "unable to create localClusterImportReconciler")
		os.Exit(1)
	}
	if err = (&localClusterImportReconciler).SetupWithManager(mgr); err != nil {
