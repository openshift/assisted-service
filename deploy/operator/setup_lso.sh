...
function install_lso() {
  ...
  catalog_source_name="redhat-operators"
    ...
    if [[ ${OC_VERSION_MAJOR_MINOR} == "4.19" && "${DISCONNECTED}" != true ]]; then
        # LSO has not been published to the 4.19 redhat-operators catalog, so
        # it cannot be installed on OpenShift 4.19. Until this is resolved,
        # we explicitly install the 4.18 catalog as redhat-operators-v4-18
        # and then subscribe to the LSO version from the 4.18 rather than the 4.19 catalog.
        # TODO: Bump the versions once LSO is published to the 4.19 catalog.
        catalog_source_name="redhat-operators-v4-18"
        oc create -f - <<EOCR
        kind: CatalogSource
        apiVersion: operators.coreos.com/v1alpha1
        metadata:
          name: redhat-operators-v4-18
