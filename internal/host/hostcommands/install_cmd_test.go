...
var _ = Describe("installcmd", func() {
	...
	BeforeEach(func() {
		...
		cluster = createClusterInDb(db, int64(3))
	...
	DescribeTable("enable MCO reboot values",
		func(enableMcoReboot bool, version string, architecture string, expected bool) {
			...
			validateInstallCommand(installCommand, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, common.TestDiskId, nil, int64(3), expected, version, true)
		},
	...
	DescribeTable("notify num reboots",
		func(notifyNumReboots bool) {
			...
			installCmdSteps, stepErr = installCommand.GetSteps(ctx, &host)
			validateInstallCommand(installCommand, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, common.TestDiskId, nil, int64(3), true, "4.15", notifyNumReboots)
		},
	...
	It("get_step_one_master_success", func() {
		...
		validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, common.TestDiskId, nil, int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
		hostFromDb := hostutil.GetHostFromDB(*host.ID, infraEnvId, db)
	...
	It("get_step_three_master_success", func() {
		...
		postvalidation(false, false, installCmdSteps[0], stepErr, models.HostRoleMaster)
		validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host2.ID, common.TestDiskId, nil, int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
		...
		validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleBootstrap, infraEnvId, clusterId, *host3.ID, common.TestDiskId, nil, int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
	})
	...
	Context("Bootable disk formatting", func() {
		...
		It("Formats a single removable disk", func() {
			...
			installCmdSteps, stepErr = installCmd.GetSteps(ctx, &host)
			postvalidation(false, false, installCmdSteps[0], stepErr, models.HostRoleMaster)
			validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, sdb.ID, getBootableDiskNames(disks), int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
			hostFromDb := hostutil.GetHostFromDB(*host.ID, infraEnvId, db)
			Expect(hostFromDb.InstallerVersion).Should(Equal(DefaultInstructionConfig.InstallerImage))
		...
		It("Formats installation disk in case it is bootable", func() {
			...
			installCmdSteps, stepErr = installCmd.GetSteps(ctx, &host)
			postvalidation(false, false, installCmdSteps[0], stepErr, models.HostRoleMaster)
			validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, sddd.ID, getBootableDiskNames(disks), int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
			hostFromDb := hostutil.GetHostFromDB(*host.ID, infraEnvId, db)
			Expect(hostFromDb.InstallerVersion).Should(Equal(DefaultInstructionConfig.InstallerImage))
			verifyDiskFormatCommand(installCmdSteps[0], sda.ID, true)
		...
		It("Format multiple bootable disks with some skipped", func() {
			...
			installCmdSteps, stepErr = installCmd.GetSteps(ctx, &host)
			postvalidation(false, false, installCmdSteps[0], stepErr, models.HostRoleMaster)
			validateInstallCommand(installCmd, installCmdSteps[0], models.HostRoleMaster, infraEnvId, clusterId, *host.ID, sdb.ID, []string{sda.ID, sdc.ID}, int64(3), false, common.TestDefaultConfig.OpenShiftVersion, true)
			hostFromDb := hostutil.GetHostFromDB(*host.ID, infraEnvId, db)
			Expect(hostFromDb.InstallerVersion).Should(Equal(DefaultInstructionConfig.InstallerImage))
			verifyDiskFormatCommand(installCmdSteps[0], sda.ID, true)
...
var _ = Describe("installcmd arguments", func() {
	...
	BeforeEach(func() {
		db, dbName = common.PrepareTestDB()
		cluster = createClusterInDb(db, int64(1))
