...
var _ = Describe("construct host install arguments", func() {
	...
	It("s390x and z/VM - static ip w/o nmstate, fcp", func() {
		...
		inventory.SystemVendor.Manufacturer = "IBM/S390"
		inventory.SystemVendor.ProductName = "z/VM    7.2.0"
		inventory.Boot.CommandLine = "rd.neednet=1 console=ttysclp0 coreos.live.rootfs_url=http://172.23.236.156:8080/assisted-installer/rootfs.img ip=10.14.6.3::10.14.6.1:255.255.255.0:master-0.boea3e06.lnxero1.boe:encbdd0:none nameserver=10.14.6.1 ip=[fd00::3]::[fd00::1]:64::encbdd0:none nameserver=[fd00::1] zfcp.allow_lun_scan=0 rd.znet=qeth,0.0.bdd0,0.0.bdd1,0.0.bdd2,layer2=1 rd.zfcp=0.0.8002,0x500507630400d1e3,0x4000404600000000 rd.dasd=0.0.5236 random.trust_cpu=on rd.luks.options=discard ignition.firstboot ignition.platform.id=metal console=tty1 console=ttyS1,115200n8"
	...
	It("s390x and KVM - DHCP", func() {
		...
		inventory.Boot.CommandLine = "coreos.liveiso=rhcos-413.92.202307260246-0 ignition.firstboot ignition.platform.id=metal"
