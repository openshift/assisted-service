...
var _ = Describe("update_role", func() {
	...
	BeforeEach(func() {
		...
		state = NewManager(getTestLog(), db, nil, nil, nil, createValidatorCfg(), nil, getHostConfig())
...
var _ = Describe("update_progress", func() {
	...
	BeforeEach(func() {
		...
		state = NewManager(getTestLog(), db, mockEvents, nil, nil, createValidatorCfg(), mockMetric, getHostConfig())
		id := strfmt.UUID(uuid.New().String())
		clusterId := strfmt.UUID(uuid.New().String())
...
var _ = Describe("monitor_disconnection", func() {
	...
	BeforeEach(func() {
		...
		ctrl = gomock.NewController(GinkgoT())
		mockEvents = events.NewMockHandler(ctrl)
		state = NewManager(getTestLog(), db, mockEvents, nil, nil, createValidatorCfg(), nil, getHostConfig())
		clusterID := strfmt.UUID(uuid.New().String())
...
var _ = Describe("cancel installation", func() {
	...
	BeforeEach(func() {
		...
		state = NewManager(getTestLog(), db, eventsHandler, nil, nil, nil, nil, getHostConfig())
		id := strfmt.UUID(uuid.New().String())
		clusterId := strfmt.UUID(uuid.New().String())
...
var _ = Describe("UpdateInventory", func() {
	...
	BeforeEach(func() {
		db = common.PrepareTestDB(dbName, &events.Event{})
		hapi = NewManager(getTestLog(), db, nil, nil, nil, createValidatorCfg(), nil, getHostConfig())
...
var _ = Describe("SetBootstrap", func() {
	...
	BeforeEach(func() {
		...
		ctrl = gomock.NewController(GinkgoT())
		mockEvents = events.NewMockHandler(ctrl)
		hapi = NewManager(getTestLog(), db, mockEvents, nil, nil, createValidatorCfg(), nil, getHostConfig())
		hostId = strfmt.UUID(uuid.New().String())
		clusterId = strfmt.UUID(uuid.New().String())
