...
var _ = Describe("HostInstallationFailed", func() {
	...
	BeforeEach(func() {
		...
		mockEvents = events.NewMockHandler(ctrl)
		hapi = NewManager(getTestLog(), db, mockEvents, nil, nil, createValidatorCfg(), mockMetric, getHostConfig())
		hostId = strfmt.UUID(uuid.New().String())
		clusterId = strfmt.UUID(uuid.New().String())
...
var _ = Describe("Cancel host installation", func() {
	...
	BeforeEach(func() {
		...
		hapi = NewManager(getTestLog(), db, mockEventsHandler, nil, nil, createValidatorCfg(), nil, getHostConfig())
	})
