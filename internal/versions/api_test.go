	...
	It("release image without cpu_architectures field", func() {
		releaseImages := models.ReleaseImages{
			...
			&models.ReleaseImage{
				...
				URL:              swag.String("release_4.13.1"),
				Default:          true,
				Version:          swag.String("4.13.1-chocobomb-for-test"),
			},
		}
		...
		version := val.Payload["4.13.1"]
		Expect(version.CPUArchitectures).Should(ContainElement(common.X86CPUArchitecture))
		Expect(version.DisplayName).Should(Equal(swag.String("4.13.1-chocobomb-for-test")))
	...
	It("release image without matching OS image", func() {
		releaseImages := models.ReleaseImages{
			&models.ReleaseImage{
				...
				URL:              swag.String("release_4.13.1"),
				Default:          true,
				Version:          swag.String("4.13.1-chocobomb-for-test-multi"),
			},
		}
		osImages := readDefaultOsImages()
		versionsHandler, err := NewHandler(logger, mockRelease, releaseImages, nil, "", nil)
		...
		Expect(version.DisplayName).Should(Equal(swag.String("4.13.1-chocobomb-for-test-multi")))
		Expect(version.Default).Should(Equal(true))
	})
