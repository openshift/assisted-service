...
var _ = Describe("getMustGatherImages", func() {
	...
	It("not hitting the cache for different openshift version (same major & minor but different patch)", func() {
		getReleaseImageLocalMock := func(_ context.Context, openshiftVersion, cpuArch, _ string) (*models.ReleaseImage, error) {
			switch openshiftVersion {
			case "4.20.1":
				return &models.ReleaseImage{URL: swag.String("release_4.20.1")}, nil
			case "4.20.2":
				return &models.ReleaseImage{URL: swag.String("release_4.20.2")}, nil
			default:
				return nil, errors.New("No release image found")
			}
		}

		mockRelease.EXPECT().GetMustGatherImage(gomock.Any(), "release_4.20.1", mirror, pullSecret).Return("image4.20.1", nil).Times(1)
		mockRelease.EXPECT().GetMustGatherImage(gomock.Any(), "release_4.20.2", mirror, pullSecret).Return("image4.20.2", nil).Times(1)

		images1, err := getMustGatherImages(
			log,
			"4.20.1",
			cpuArchitecture,
			pullSecret,
			mirror,
			cache,
			getReleaseImageLocalMock,
			mockRelease,
		)
		Expect(err).ShouldNot(HaveOccurred())
		Expect(images1["ocp"]).To(Equal("image4.20.1"))

		images2, err := getMustGatherImages(
			log,
			"4.20.2",
			cpuArchitecture,
			pullSecret,
		...
		Expect(err).ShouldNot(HaveOccurred())
		Expect(images2["ocp"]).To(Equal("image4.20.2"))
	})
