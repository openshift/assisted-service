...
var _ = Describe("list versions", func() {
	...
	It("default values", func() {
		...
		Expect(val.Payload["assisted-installer-service"]).
			Should(Equal("quay.io/ocpmetal/installer-image-build:stable"))
		Expect(val.Payload["image-builder"]).Should(Equal("quay.io/ocpmetal/installer-image-build:stable"))
		Expect(val.Payload["discovery-agent"]).Should(Equal("quay.io/ocpmetal/agent:stable"))
		Expect(val.Payload["ignition-manifests-and-kubeconfig-generate"]).
			Should(Equal("quay.io/ocpmetal/ignition-manifests-and-kubeconfig-generate:stable"))
		Expect(val.Payload["assisted-installer"]).Should(Equal("quay.io/ocpmetal/assisted-installer:stable"))
	})
	...
	It("mix default and non default", func() {
		...
		Expect(reply).Should(BeAssignableToTypeOf(operations.NewListComponentVersionsOK()))
		val, _ := reply.(*operations.ListComponentVersionsOK)
		Expect(val.Payload["assisted-installer-service"]).Should(Equal("self-version"))
		Expect(val.Payload["image-builder"]).Should(Equal("image-builder"))
		Expect(val.Payload["discovery-agent"]).Should(Equal("agent-image"))
		Expect(val.Payload["ignition-manifests-and-kubeconfig-generate"]).
			Should(Equal("quay.io/ocpmetal/ignition-manifests-and-kubeconfig-generate:stable"))
		Expect(val.Payload["assisted-installer"]).Should(Equal("installer-image"))
	})
})
