...
var _ = Describe("ClusterManifestTests", func() {
	...
	mockUpload := func(times int) {
		mockS3Client.EXPECT().Upload(ctx, gomock.Any(), gomock.Any(), gomock.Any()).Return(nil).AnyTimes()
	}
	...
	mockListByPrefix := func(clusterID *strfmt.UUID, files []s3wrapper.ObjectInfo) {
		mockS3Client.EXPECT().ListObjectsByPrefix(ctx, filepath.Join(clusterID.String(), constants.ManifestMetadataFolder)).Return([]s3wrapper.ObjectInfo{}, nil).Times(1)
...
`)
	...
	Context("V2ListClusterManifests", func() {
		...
		It("should not filter system manifests if requested to show all", func() {
			...
			systemGenerated := constants.ManifestSourceSystemGenerated
			manifests := []models.Manifest{
				...
				},
				{
					...
					Folder:         defaultFolder,
					ManifestSource: &systemGenerated,
				},
				{
		...
		It("list manifests for new cluster", func() {
			clusterID := registerCluster().ID
			mockListByPrefix(clusterID, []s3wrapper.ObjectInfo{})
			response := manifestsAPI.V2ListClusterManifests(ctx, operations.V2ListClusterManifestsParams{
				ClusterID: *clusterID,
			})
			Expect(response).Should(BeAssignableToTypeOf(operations.NewV2ListClusterManifestsOK()))
	...
	Context("UpdateClusterManifest", func() {
		...
		It("moves existing file from one path to another", func() {
			...
			mockS3Client.EXPECT().Upload(ctx, []byte(contentAsYAML), getObjectName(clusterID, destFolder, destFileName), gomock.Any()).Return(nil).Times(1)
		...
		It("Should use the source file name when the updated file name is omitted ", func() {
			...
			reader := io.NopCloser(strings.NewReader(contentAsYAML))
			mockS3Client.EXPECT().Download(ctx, getObjectName(clusterID, defaultFolder, fileNameYaml)).Return(reader, int64(0), nil).Times(1)
			mockS3Client.EXPECT().Upload(ctx, []byte(contentAsYAML), getObjectName(clusterID, destFolder, fileNameYaml), gomock.Any()).Return(nil).Times(1)
			mockS3Client.EXPECT().DoesObjectExist(ctx, getObjectName(clusterID, defaultFolder, fileNameYaml)).Return(true, nil).Times(1)
		...
		It("Should use the source folder when updated folder is omitted", func() {
			...
			reader := io.NopCloser(strings.NewReader(contentAsYAML))
			mockS3Client.EXPECT().Download(ctx, getObjectName(clusterID, defaultFolder, fileNameYaml)).Return(reader, int64(0), nil).Times(1)
			mockS3Client.EXPECT().Upload(ctx, []byte(contentAsYAML), getObjectName(clusterID, defaultFolder, destFileName), gomock.Any()).Return(nil).Times(1)
	...
	Describe("ListClusterManifestsInternal", func() {
		...
		It("Should check legacy filesystem metadata if no S3/Xattr metata present", func() {
			...
			mockS3Client.EXPECT().ListObjectsByPrefix(ctx, filepath.Join(clusterId.String(), constants.ManifestMetadataFolder)).Return(
				[]s3wrapper.ObjectInfo{{
					Path:     filepath.Join(clusterId.String(), constants.ManifestMetadataFolder, "openshift", "no-new-metadata.yaml", constants.ManifestSourceUserSupplied),
					Metadata: make(map[string]string),
				}}, nil).Times(1)
			objectName := filepath.Join(clusterId.String(), constants.ManifestFolder)
			mockS3Client.EXPECT().ListObjectsByPrefix(ctx, objectName).Return(manifests, nil).Times(1)
