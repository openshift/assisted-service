...
func (m *Manifests) CreateClusterManifestInternal(ctx context.Context, params operations.V2CreateClusterManifestParams, isCustomManifest bool) (*models.Manifest, error) {
	...
	generatedBy := constants.ManifestSourceSystemGenerated
	if isCustomManifest {
		generatedBy = constants.ManifestSourceUserSupplied
	}

	err = m.uploadManifest(ctx, manifestContent, params.ClusterID, path, generatedBy)
	if err != nil {
		return nil, err
	}

	log.Infof("Done creating manifest %s for cluster %s", path, params.ClusterID.String())
	manifest := models.Manifest{FileName: fileName, Folder: folder, ManifestSource: &generatedBy}
	return &manifest, nil
}
...
func (m *Manifests) FindUserManifestPathsByLegacyMetadata(ctx context.Context, clusterID strfmt.UUID) ([]string, error) {
	...
	objects, err := m.objectHandler.ListObjectsByPrefix(ctx, objectName)
	if err != nil {
	...
	for _, file := range objects {
		...
		// file path for metadata path would be {cluster_id}/manifests/manifests/some-manifest.yml/user-supplied
		pathParts := strings.Split(file.Path, "/")
		if len(pathParts) != 5 || pathParts[4] != constants.ManifestSourceUserSupplied {
		...
		manifestFilePath := strings.Join(pathParts[0:len(pathParts)-1], "/")
...
func (m *Manifests) ListClusterManifestsInternal(ctx context.Context, params operations.V2ListClusterManifestsParams) (models.ListManifests, error) {
	...
	for _, file := range files {
		...
		// If we found no manifest source information in metadata, check the filesystem (legacy method) for an override.
		if !ok && swag.ContainsStrings(legacyUserManifestPaths, file.Path) {
...
func (m *Manifests) UpdateClusterManifestInternal(ctx context.Context, params operations.V2UpdateClusterManifestParams) (*models.Manifest, error) {
	...
	manifest := models.Manifest{FileName: destFileName, Folder: destFolder, ManifestSource: &userSupplied}
	return &manifest, nil
}
...
func GetClusterManifests(ctx context.Context, clusterID *strfmt.UUID, s3Client s3wrapper.API) ([]s3wrapper.ObjectInfo, error) {
	...
	if err != nil {
		return []s3wrapper.ObjectInfo{}, err
	}
...
func (m *Manifests) uploadManifest(ctx context.Context, content []byte, clusterID strfmt.UUID, path string, generatedBy string) error {
	objectName := GetManifestObjectName(clusterID, path)
	metadata := map[string]string{constants.ManifestSourceAttribute: generatedBy}
	if err := m.objectHandler.Upload(ctx, content, objectName, metadata); err != nil {
