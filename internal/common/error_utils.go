...
//NotFound represents an Http 404 error that is used in the filesystem module.
//It implements the builtin error interface and should be replaced with the more
//commonly used ApiErrorResponse
...
// The ServiceUnavailableResponse interface type is the conventional interface
// for representing an Http 503 error thrown from the REST API. It implements
// both middleware.Responder interface and the builtin error interface.
// It structure is a free form string so it conforms with 503 error structure
// as generated by the OCP router
type ServiceUnavailableResponse string

func (sur *ServiceUnavailableResponse) Error() string {
	return string(*sur)
}

func (sur *ServiceUnavailableResponse) WriteResponse(rw http.ResponseWriter, producer runtime.Producer) {
	rw.WriteHeader(http.StatusServiceUnavailable)
	if err := producer.Produce(rw, string(*sur)); err != nil {
		panic(err) // let the recovery middleware deal with this
	}
}

func NewServiceUnavailableError(err error) *ServiceUnavailableResponse {
	var value ServiceUnavailableResponse
	if err == nil {
		value = ServiceUnavailableResponse("")
	} else {
		value = ServiceUnavailableResponse(err.Error())
	}
	return &value
}

// The ApiErrorResponse interface type is the conventional interface
// for representing an application-level error in the REST API.
// It implements both middleware.Responder interface and the builtin
// error interface.
...
// The InfraErrorResponse interface type is the conventional interface
// for representing 4xx errors in the REST API. It implements both
// middleware.Responder interface and the builtin error interface.
// It structure is different from the API response so it conforms with
// 4xx errors comming from the OpenShift infrastructure
