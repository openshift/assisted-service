...
var _ = Describe("TestVersionBuilder", func() {
	...
	Describe("Constraints", func() {
		...
		Describe("constraint replaces previous constraint", func() {
			It("uses the last constraint set", func() {
				withVersions(map[string][]string{
					X86CPUArchitecture: {"4.14", "4.15", "4.16", "4.17"},
				}, func() {
					v := TestVersion().LessThan("4.15").GreaterThan("4.15").Version()
					Expect(v).To(Equal("4.17"))
				})
			})
		})
