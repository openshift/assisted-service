...
var _ = Describe("DatabaseImage", func() {
	...
	It("returns DATABASE_IMAGE when it is set", func() {
		Expect(os.Setenv(key, "registry.example/olm/sclorg-postgresql-12-c8s:latest")).To(Succeed())
		Expect(DatabaseImage()).To(Equal("registry.example/olm/sclorg-postgresql-12-c8s:latest"))
	})
})
