...
var _ = Describe("agentserviceconfig_controller with disable-image-service annotation", func() {
	var (
		asc                             *aiv1beta1.AgentServiceConfig
		ascr                            *AgentServiceConfigReconciler
		agentinstalladmissionDeployment *appsv1.Deployment
	...
	BeforeEach(func() {
		asc = newASCDefault()
		// Add the disable-image-service annotation
		asc.ObjectMeta.Annotations = map[string]string{
			"unsupported.agent-install.openshift.io/disable-image-service": "true",
		}
	})

	It("should not deploy image service resources when disable-image-service annotation is set", func() {
		agentinstalladmissionDeployment = &appsv1.Deployment{
			ObjectMeta: metav1.ObjectMeta{
				Name:      "agentinstalladmission",
				Namespace: testNamespace,
		...
		// Verify that DISABLE_IMAGE_SERVICE env var is set in the ConfigMap (loaded via envFrom)
		...
		Expect(assistedCMFound.Data["DISABLE_IMAGE_SERVICE"]).To(Equal("true"), "DISABLE_IMAGE_SERVICE should be set to 'true' in the ConfigMap")
	...
	It("should set empty OS_IMAGES environment variable when image service is disabled", func() {
		...
		// Also verify that DISABLE_IMAGE_SERVICE env var is set in the ConfigMap (already checked above)
		Expect(assistedCMFound.Data["DISABLE_IMAGE_SERVICE"]).To(Equal("true"), "DISABLE_IMAGE_SERVICE should be set to 'true' in the ConfigMap")
