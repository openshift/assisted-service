...
const (
	...
	disableImageServiceAnnotation       = "unsupported.agent-install.openshift.io/disable-image-service"
...
func (r *AgentServiceConfigReconciler) Reconcile(origCtx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	// Clear OSImages in spec if image service is disabled via annotation
	if isImageServiceDisabled(asc.Object.GetAnnotations()) && asc.spec.OSImages != nil {
		log.Info("Image service is disabled, clearing OSImages from spec")
		asc.spec.OSImages = nil
		// Update the spec to reflect the change
		if err := r.Update(ctx, asc.Object); err != nil {
			log.WithError(err).Error("Failed to update AgentServiceConfig spec")
			return ctrl.Result{Requeue: true}, err
		}
	}
	...
	if !isImageServiceDisabled(asc.Object.GetAnnotations()) {
...
func getComponents(spec *aiv1beta1.AgentServiceConfigSpec, isOpenshift bool, annotations map[string]string) []component {
	imageServiceDisabled := isImageServiceDisabled(annotations)
...
func monitorOperands(ctx context.Context, log logrus.FieldLogger, asc ASC) (string, error) {
	...
	// monitor statefulset (only if image service is not disabled)
	if !isImageServiceDisabled(asc.Object.GetAnnotations()) {
...
func newAssistedCM(ctx context.Context, log logrus.FieldLogger, asc ASC) (client.Object, controllerutil.MutateFn, error) {
	...
	mutateFn := func() error {
		...
		// Set DISABLE_IMAGE_SERVICE environment variable if the annotation is set
		if isImageServiceDisabled(asc.Object.GetAnnotations()) {
			cm.Data["DISABLE_IMAGE_SERVICE"] = "true"
			log.Infof("Image service disabled via annotation, setting DISABLE_IMAGE_SERVICE=true")
			asc.rec.Recorder.Event(asc.Object, "Normal", "ImageServiceDisabled", "Image service has been disabled via unsupported annotation")
		}
...
func getOSImages(log logrus.FieldLogger, spec *aiv1beta1.AgentServiceConfigSpec, annotations map[string]string) string {
	...
	if isImageServiceDisabled(annotations) {
...
// isImageServiceDisabled returns true if the disable-image-service annotation is set to "true"
func isImageServiceDisabled(annotations map[string]string) bool {
	if annotations == nil {
		return false
	}
