...
type SpokeClientCache interface {
	Get(ctx context.Context, secret *corev1.Secret) (spoke_k8s_client.SpokeK8sClient, error)
}
...
func (c *spokeClientCache) Get(ctx context.Context, secret *corev1.Secret) (spoke_k8s_client.SpokeK8sClient, error) {
	c.Lock()
	defer c.Unlock()
	...
	if !present || client.secretHash != secretHash {
		spokeK8sClient, err := c.clientFactory.CreateFromSecret(ctx, secret)
		if err != nil {
