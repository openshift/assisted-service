...
func (r *PreprovisioningImageReconciler) handleImageUpdate(ctx context.Context, log logrus.FieldLogger, image *metal3_v1alpha1.PreprovisioningImage, infraEnv *aiv1beta1.InfraEnv) error {
	...
	imageUpdated := false
	if bmh.Status.Provisioning.State == metal3_v1alpha1.StateProvisioned {
		log.Info("Ignoring image update for provisioned BMH")
		setProvisionedCondition(image)
	} else {
...
func setProvisionedCondition(image *metal3_v1alpha1.PreprovisioningImage) {
	message := "BareMetalHost is already provisioned"
