	...
	Describe("Reconcile a BMH with a non-approved matching agent", func() {
		...
		Context("when an agent matches", func() {
			...
			Context("should set the agent labels based on the corresponding BMH annotations", func() {
				annotationValue := func(operation, value string) string {
					return fmt.Sprintf(`{"operation": "%s", "value": "%s"}`, operation, value)
				}
				...
				addLabel := func(bmh *bmh_v1alpha1.BareMetalHost, key, value string) {
					addAnnotation(bmh, AGENT_LABEL_PREFIX+key, annotationValue("add", value))
				}
				deleteLabel := func(bmh *bmh_v1alpha1.BareMetalHost, key string) {
					addAnnotation(bmh, AGENT_LABEL_PREFIX+key, annotationValue("delete", ""))
				}
				...
				It("modify node labels", func() {
					...
					expectToNotContainKey(updatedAgent, "initial-key")

					Expect(c.Get(ctx, types.NamespacedName{Name: host.Name, Namespace: host.Namespace}, h2)).ToNot(HaveOccurred())
					deleteLabel(h2, "third-label")
					...
					expectToNotContainKey(updatedAgent, "third-label")
				})
			})
