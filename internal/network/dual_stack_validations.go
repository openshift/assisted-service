...
// DeterminePrimaryIPStack analyzes the provided networks and VIPs to determine
// the primary IP stack based on which IP family appears first in the lists
func DeterminePrimaryIPStack(
	...
	for _, ip := range firstIPs {
		...
		if IsIPV4CIDR(ip) || IsIPv4Addr(ip) {
			...
			continue // Skip invalid IPs - don't increment validIPCount
		}
...
// IPv6-primary dual-stack is supported starting from OCP 4.13
...
func ValidateDualStackOrder(
	...
	if allowIPv6Primary {
		// For OCP 4.13+: Allow any order, just ensure we have one IPv4 and one IPv6
		hasIPv4 := false
		hasIPv6 := false
		...
		// For OCP < 4.13: Maintain original IPv4-first requirement
		if !isIPv4Func(items[0]) {
			return errors.Errorf("First %s has to be IPv4 %s (IPv6-primary dual-stack requires OpenShift 4.13+), got %s", itemType, itemUnit, items[0])
		}
...
//   - for OCP < 4.13: the first one is IPv4 subnet and the second one is IPv6 subnet
//   - for OCP >= 4.13: one is IPv4 subnet and one is IPv6 subnet (in any order)
