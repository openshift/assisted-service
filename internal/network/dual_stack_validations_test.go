...
var _ = Describe("DualStack Primary IP Stack Functionality", func() {
	...
	Describe("DeterminePrimaryIPStack", func() {
		It("should return nil for empty networks", func() {
			primaryStack, err := DeterminePrimaryIPStack(nil, nil, nil, nil, nil)
		...
		It("should return IPv4 primary for IPv4-first dual stack", func() {
			...
			primaryStack, err := DeterminePrimaryIPStack(machineNetworks, apiVips, ingressVips, serviceNetworks, clusterNetworks)
			Expect(err).ToNot(HaveOccurred())
		...
		It("should return error for inconsistent IP family order", func() {
			...
			primaryStack, err := DeterminePrimaryIPStack(machineNetworks, apiVips, nil, nil, nil)
	...
	Describe("ValidateDualStackOrder", func() {
		Context("OCP 4.13+ (IPv6-primary support)", func() {
			It("should accept IPv4-first order", func() {
				items := []string{"10.0.0.0/16", "2001:db8::/64"}
				err := ValidateDualStackOrder(items, "machine networks", "subnet", "4.13.0", IsIPV4CIDR, IsIPv6CIDR)
				Expect(err).ToNot(HaveOccurred())
			})
		...
		Context("OCP < 4.13 (IPv4-first only)", func() {
			It("should accept IPv4-first order", func() {
				items := []string{"10.0.0.0/16", "2001:db8::/64"}
			...
			It("should reject IPv6-first order", func() {
				...
				err := ValidateDualStackOrder(items, "machine networks", "subnet", "4.12.0", IsIPV4CIDR, IsIPv6CIDR)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(ContainSubstring("First machine networks has to be IPv4 subnet (IPv6-primary dual-stack requires OpenShift 4.13+)"))
			})
	...
	Describe("Updated validation functions", func() {
		Context("VerifyMachineNetworksDualStack", func() {
			...
			It("should fail for IPv6-first on OCP 4.12", func() {
				networks := []*models.MachineNetwork{
					{Cidr: "2001:db8::/64"},
					{Cidr: "10.0.0.0/16"},
				}
			...
			It("should pass for IPv6-first on OCP 4.13", func() {
				...
				err := VerifyMachineNetworksDualStack(networks, true, "4.13.0")
				Expect(err).ToNot(HaveOccurred())
			})
