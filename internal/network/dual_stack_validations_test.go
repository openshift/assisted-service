...
var _ = Describe("DualStack Primary IP Stack Functionality", func() {
	...
	Describe("SetPrimaryIPStack", func() {
		It("should return nil for empty networks", func() {
			primaryStack, err := SetPrimaryIPStack(nil, nil, nil, nil, nil)
		...
		It("should return IPv4 primary for IPv4-first dual stack", func() {
			...
			primaryStack, err := SetPrimaryIPStack(machineNetworks, apiVips, ingressVips, serviceNetworks, clusterNetworks)
			Expect(err).ToNot(HaveOccurred())
		...
		It("should return error for inconsistent IP family order", func() {
			...
			primaryStack, err := SetPrimaryIPStack(machineNetworks, apiVips, nil, nil, nil)
