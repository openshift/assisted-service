...
func NewManager(log logrus.FieldLogger, manifestAPI manifestsapi.ManifestsAPI, options Options, objectHandler s3wrapper.API) *Manager {
	return NewManagerWithOperators(log, manifestAPI, options, objectHandler, lso.NewLSOperator(), odf.NewOcsOperator(log), odf.NewOdfOperator(log), cnv.NewCNVOperator(log, options.CNVConfig), lvm.NewLvmOperator(log), mce.NewMceOperator(log), mtv.NewMTVOperator(log), osc.NewOscOperator(log))
}
