...
const (
	clusterValidationID = string(models.ClusterValidationIDLokiRequirementsSatisfied)
	hostValidationID    = string(models.HostValidationIDLokiRequirementsSatisfied)
)
...
// operator is a Loki OLM operator plugin; it implements api.Operator
type operator struct {
	log       logrus.FieldLogger
	templates *template.Template
}

// NewLokiOperator creates new Loki Operator
func NewLokiOperator(log logrus.FieldLogger) *operator {
...
func (o *operator) ValidateCluster(ctx context.Context, cluster *common.Cluster) ([]api.ValidationResult, error) {
	...
	if ok, _ := common.BaseVersionLessThan(LokiMinOpenshiftVersion, cluster.OpenshiftVersion); ok {
		result.Status = api.Failure
		result.Reasons = []string{
			fmt.Sprintf("Loki Operator is only supported for OpenShift versions %s and above", LokiMinOpenshiftVersion),
		}
	}
...
// GetMonitoredOperator returns MonitoredOperator corresponding to the Loki Operator
