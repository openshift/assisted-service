...
var _ = Describe("Loki manifest generation", func() {
	var (
	...
	BeforeEach(func() {
		...
		operator = NewLokiOperator(common.GetTestLog())
	})
	...
	It("Namespace manifest has correct content", func() {
		...
		nsManifest := manifests["50_openshift-loki_ns.yaml"]
	...
	It("Subscription manifest has correct content", func() {
		...
		subManifest := manifests["50_openshift-loki_subscription.yaml"]
