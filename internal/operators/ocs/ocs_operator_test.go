	...
	Context("GetHostRequirements", func() {
		table.DescribeTable("compact mode scenario: get requirements for hosts when ", func(cluster *common.Cluster, host *models.Host, expectedResult *models.ClusterHostRequirementsDetails) {
			...
			table.Entry("Single master",
				...
				masterWithThreeDisk,
				&models.ClusterHostRequirementsDetails{CPUCores: CPUCompactMode + 2*operator.config.OCSPerDiskCPUCount, RAMMib: conversions.GibToMib(MemoryGiBCompactMode + 2*operator.config.OCSPerDiskRAMGiB), DiskSizeGb: MinDiskSize},
			),
			...
			table.Entry("no disk in one of the master",
				...
				masterWithNoDisk,
				&models.ClusterHostRequirementsDetails{CPUCores: CPUCompactMode + operator.config.OCSPerDiskCPUCount, RAMMib: conversions.GibToMib(MemoryGiBCompactMode + operator.config.OCSPerDiskRAMGiB), DiskSizeGb: MinDiskSize},
			),
			...
			table.Entry("there are two master and one worker",
				...
				workerWithTwoDisk,
				&models.ClusterHostRequirementsDetails{CPUCores: CPUMinimalMode + operator.config.OCSPerDiskCPUCount, RAMMib: conversions.GibToMib(MemoryGiBMinimalMode + operator.config.OCSPerDiskRAMGiB), DiskSizeGb: MinDiskSize},
			),
		)
		...
		table.DescribeTable("standard and minimal mode scenario: get requirements for hosts when ", func(cluster *common.Cluster, host *models.Host, expectedResult *models.ClusterHostRequirementsDetails) {
			...
			table.Entry("there are 6 hosts, worker with three disk requirements",
				...
				workerWithThreeDisk,
				&models.ClusterHostRequirementsDetails{CPUCores: CPUMinimalMode + 2*operator.config.OCSPerDiskCPUCount, RAMMib: conversions.GibToMib(MemoryGiBMinimalMode + 2*operator.config.OCSPerDiskRAMGiB), DiskSizeGb: MinDiskSize},
			),
