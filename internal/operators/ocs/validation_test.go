	...
	tests := []struct {
		...
		},
		{
			...
			validationsChecker: makeJsonChecker(map[clust.ValidationID]validationCheckResult{
				...
				clust.IsPullSecretSet:                     {status: clust.ValidationSuccess, messagePattern: "The pull secret is set"},
				clust.SufficientMastersCount:              {status: clust.ValidationSuccess, messagePattern: "The cluster has a sufficient number of master candidates."},
				clust.IsOcsRequirementsSatisfied:          {status: clust.ValidationFailure, messagePattern: "Insufficient Resources to deploy OCS in Compact Mode. A minimum of 3 Disks of minimum 25GB, 3 Hosts with disks, is required."},
			}),
			errorExpected: false,
		},
		{
		...
		},
		{
			...
			validationsChecker: makeJsonChecker(map[clust.ValidationID]validationCheckResult{
				...
				clust.IsPullSecretSet:                     {status: clust.ValidationSuccess, messagePattern: "The pull secret is set"},
				clust.SufficientMastersCount:              {status: clust.ValidationSuccess, messagePattern: "The cluster has a sufficient number of master candidates."},
				clust.IsOcsRequirementsSatisfied:          {status: clust.ValidationFailure, messagePattern: "Insufficient Resources to deploy OCS in Standard Mode. A minimum of 3 Disks of minimum 25GB, 3 Hosts with disks, is required."},
			}),
			errorExpected: false,
		},
		{
