...
var _ = Describe("Manifest generation", func() {
	...
	BeforeEach(func() {
		...
		operator = NewNodeMaintenanceOperator(common.GetTestLog())
	})

	It("Generates the required manifests", func() {
		manifests, _, err := operator.GenerateManifests(cluster)
		Expect(err).ToNot(HaveOccurred())
		Expect(manifests).To(HaveKey("50_node_maintenance_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_node_maintenance_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_node_maintenance_operatorgroup.yaml"))
	})
