...
var _ = Describe("CNV operator", func() {
	...
	DescribeTable("getDependencies", func(ocpVersion string, haMode int64, expectedOperator string) {
		...
		Entry("LSO, Multi node 4.15", "4.15", int64(3), lso.Operator.Name),
		Entry("LSO, Multi node 4.21", "4.21", int64(3), lso.Operator.Name),
		Entry("LSO, Multi node 4.12", "4.12", int64(3), lso.Operator.Name),
	...
	Context("host requirements", func() {
		...
		BeforeEach(func() {
			cluster = common.Cluster{
				Cluster: models.Cluster{ControlPlaneCount: int64(3), OpenshiftVersion: lvm.LvmsMinOpenshiftVersion4_12},
			}
		})
		...
		It("should return reqs for SNO", func() {
			...
			cluster = common.Cluster{
				Cluster: models.Cluster{ControlPlaneCount: int64(1), OpenshiftVersion: lvm.LvmsMinOpenshiftVersion4_12},
			}
	...
	Context("ValidateHost", func() {
		...
		DescribeTable("validateHost when ", func(cluster *common.Cluster, host *models.Host, expectedResult api.ValidationResult) {
			...
			Entry("SNO and there is no disk with bigger size than threshold for HPP",
				&common.Cluster{Cluster: models.Cluster{OpenshiftVersion: "4.10", ControlPlaneCount: noneHaMode, Hosts: []*models.Host{masterWithLessDiskSizeAndVirt}}},
			...
			Entry("SNO and there is a disk with bigger size than threshold for HPP",
				&common.Cluster{Cluster: models.Cluster{OpenshiftVersion: "4.10", ControlPlaneCount: noneHaMode, Hosts: []*models.Host{masterWithOneSatisfyingDiskAndVirt}}},
			...
			Entry("Non SNO and there is no disk with bigger size than threshold for HPP shouldn't bother us",
				&common.Cluster{Cluster: models.Cluster{OpenshiftVersion: "4.10", ControlPlaneCount: fullHaMode, Hosts: []*models.Host{masterWithLessDiskSizeAndVirt}}},
				masterWithLessDiskSizeAndVirt,
				api.ValidationResult{Status: api.Success, ValidationId: cnvOperator.GetHostValidationID(), Reasons: nil},
			),
		)
