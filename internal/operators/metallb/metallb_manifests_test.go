...
var _ = Describe("Manifest generation", func() {
	...
	It("Generates the required manifests", func() {
		...
		Expect(manifests).To(HaveKey("50_metallb_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_metallb_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_metallb_operatorgroup.yaml"))
	})
