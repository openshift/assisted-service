	...
	Context("ValidateHost", func() {
		table.DescribeTable("validate host when ", func(cluster *common.Cluster, host *models.Host, expectedResult api.ValidationResult) {
			...
			table.Entry("host with insufficient disks",
				...
				api.ValidationResult{Status: api.Failure, ValidationId: operator.GetHostValidationID(), Reasons: []string{"ODF LVM requires at least one non-installation HDD/SSD disk on the host"}},
			),
	...
	Context("ValidateCluster", func() {
		...
		table.DescribeTable("validate cluster when ", func(cluster *common.Cluster, expectedResult api.ValidationResult) {
			...
			table.Entry("High Availability Mode Full",
				...
				api.ValidationResult{Status: api.Failure, ValidationId: operator.GetHostValidationID(), Reasons: []string{"ODF LVM operator is only supported for Single Node Openshift"}},
			),
			table.Entry("High Availability Mode None and Openshift version less than minimal",
				...
				api.ValidationResult{Status: api.Failure, ValidationId: operator.GetHostValidationID(), Reasons: []string{"ODF LVM operator is only supported for openshift versions 4.11.0 and above"}},
			),
