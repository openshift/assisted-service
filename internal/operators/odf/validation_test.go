	...
	tests := []struct {
	...
	}{
		{
			...
			validationsChecker: makeJsonChecker(map[clust.ValidationID]validationCheckResult{
				...
				clust.SufficientMastersAndWorkersCount:    {status: clust.ValidationSuccess, messagePattern: fmt.Sprintf("The cluster has between %d-%d dedicated control plane nodes.", common.MinMasterHostsNeededForInstallationInHaMode, common.MaxMasterHostsNeededForInstallationInHaMode)},
		...
		},
		{
			...
			validationsChecker: makeJsonChecker(map[clust.ValidationID]validationCheckResult{
				...
				clust.IsDNSDomainDefined:                  {status: clust.ValidationSuccess, messagePattern: "The base domain is defined"},
				clust.IsPullSecretSet:                     {status: clust.ValidationSuccess, messagePattern: "The pull secret is set"},
				clust.SufficientMastersAndWorkersCount:    {status: clust.ValidationFailure, messagePattern: fmt.Sprintf("Clusters must have between %d-%d dedicated control plane nodes. Add or remove hosts, or change their roles configurations to meet the requirement.", common.MinMasterHostsNeededForInstallationInHaMode, common.MaxMasterHostsNeededForInstallationInHaMode)},
