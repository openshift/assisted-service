...
var _ = Describe("Manifest generation", func() {
	...
	BeforeEach(func() {
		...
		operator = NewClusterObservabilityOperator(common.GetTestLog())
	})

	It("Generates the required manifests", func() {
		manifests, _, err := operator.GenerateManifests(cluster)
		Expect(err).ToNot(HaveOccurred())
		Expect(manifests).To(HaveKey("50_cluster_observability_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_cluster_observability_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_cluster_observability_operatorgroup.yaml"))
	})
