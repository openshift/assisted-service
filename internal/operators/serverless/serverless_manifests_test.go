...
var _ = Describe("Manifest generation", func() {
	...
	BeforeEach(func() {
		...
		operator = NewServerLessOperator(common.GetTestLog())
	})

	It("Generates the required OpenShift manifests", func() {
		manifests, _, err := operator.GenerateManifests(cluster)
		Expect(err).ToNot(HaveOccurred())
		Expect(manifests).To(HaveKey("50_serverless_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_serverless_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_serverless_operatorgroup.yaml"))
	})
