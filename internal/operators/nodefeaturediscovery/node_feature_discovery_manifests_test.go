...
var _ = Describe("Manifest generation", func() {
	...
	BeforeEach(func() {
		...
		operator = NewNodeFeatureDiscoveryOperator(common.GetTestLog())
	})

	It("Generates the required OpenShift manifests", func() {
		manifests, _, err := operator.GenerateManifests(cluster)
		Expect(err).ToNot(HaveOccurred())
		Expect(manifests).To(HaveKey("50_nodefeaturediscovery_namespace.yaml"))
		Expect(manifests).To(HaveKey("50_nodefeaturediscovery_subscription.yaml"))
		Expect(manifests).To(HaveKey("50_nodefeaturediscovery_operatorgroup.yaml"))
	})
