...
func (g *installerGenerator) bootstrapInPlaceIgnitionsCreate(ctx context.Context, installerPath string, envVars []string) error {
	...
	bootstrapConfig, err := common.ParseIgnitionFile(bootstrapPath)
	if err != nil {
		return err
	}
	...
	for _, file := range []string{masterIgn, workerIgn} {
		err = common.WriteIgnitionFile(filepath.Join(g.workDir, file), &config)
		if err != nil {
...
func (g *installerGenerator) updateBootstrap(ctx context.Context, bootstrapPath string) error {
	...
	config, err := common.ParseIgnitionFile(bootstrapPath)
	if err != nil {
		g.log.Error(err)
		return err
	}
	...
	if g.cluster.ControlPlaneCount != 1 {
		common.SetFileInIgnition(config, "/opt/openshift/assisted-install-bootstrap", "data:,", false, 420, false)
	}
	...
	err = common.WriteIgnitionFile(bootstrapPath, config)
	if err != nil {
		log.Error(err)
		return err
	}
...
func (g *installerGenerator) updateDhcpFiles() error {
	...
	config, err := common.ParseIgnitionFile(path)
	if err != nil {
		return err
	}
	common.SetFileInIgnition(config, "/etc/keepalived/unsupported-monitor.conf", g.encodedDhcpFileContents, false, 0o644, false)
	encodedApiVip := network.GetEncodedApiVipLease(g.cluster)
	if encodedApiVip != "" {
		common.SetFileInIgnition(config, "/etc/keepalived/lease-api", encodedApiVip, false, 0o644, false)
	}
	encodedIngressVip := network.GetEncodedIngressVipLease(g.cluster)
	if encodedIngressVip != "" {
		common.SetFileInIgnition(config, "/etc/keepalived/lease-ingress", encodedIngressVip, false, 0o644, false)
	}
	err = common.WriteIgnitionFile(path, config)
	if err != nil {
		return err
	}
	return nil
...
func (g *installerGenerator) addIpv6FileInIgnition(ignition string) error {
	...
	common.SetFileInIgnition(config, "/etc/NetworkManager/conf.d/01-ipv6.conf", encodeIpv6Contents(v6config), false, 0o644, false)
	err = common.WriteIgnitionFile(path, config)
	if err != nil {
		return err
	}
...
func (g *installerGenerator) updatePointerIgnitionMCP(poolName string, ignitionStr string) (string, error) {
	config, err := common.ParseToLatest([]byte(ignitionStr))
	if err != nil {
		return "", err
	}
...
func (g *installerGenerator) writeSingleHostFile(host *models.Host, baseFile string, workDir string) error {
	config, err := common.ParseIgnitionFile(filepath.Join(workDir, baseFile))
	if err != nil {
		return err
	}
	...
	if common.IsSingleNodeCluster(g.cluster) {
		...
		common.SetFileInIgnition(config, nodeIpHintFile, fmt.Sprintf("data:,KUBELET_NODEIP_HINT=%s", ip), false, 420, true)
		...
		if ok {
			common.SetFileInIgnition(config, nodeIpHintFile, fmt.Sprintf("data:,KUBELET_NODEIP_HINT=%s", allocation.HintIp), false, 420, true)
	...
	if host.IgnitionConfigOverrides != "" {
		merged, mergeErr := common.MergeIgnitionConfig(configBytes, []byte(host.IgnitionConfigOverrides))
		if mergeErr != nil {
	...
	if inventory.Boot != nil && inventory.Boot.DeviceType == models.BootDeviceTypePersistent {
		...
		merged, mergeErr := common.MergeIgnitionConfig(configBytes, []byte(cleanupDiscoveryStaterootIgnitionOverride))
		if mergeErr != nil {
...
func ExtractClusterID(reader io.ReadCloser) (string, error) {
	...
	config, err := common.ParseToLatest(bs)
	if err != nil {
		return "", err
	}
...
func setNMConfigration(config *config_latest_types.Config) {
	...
	common.SetFileInIgnition(config, "/etc/NetworkManager/conf.d/99-kni.conf", fileContents, false, 420, false)
}
...
func setCACertInIgnition(role models.HostRole, path string, workDir string, caCertFile string) error {
	...
	common.SetFileInIgnition(config, common.HostCACertPath, fmt.Sprintf("data:,%s", url.PathEscape(string(caCertData))), false, 420, false)

	fileName := fmt.Sprintf("%s.ign", role)
	err = common.WriteIgnitionFile(filepath.Join(workDir, fileName), config)
	if err != nil {
		return err
	}
