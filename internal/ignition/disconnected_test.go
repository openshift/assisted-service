...
var _ = Describe("Disconnected Ignition", func() {
	...
	Context("GenerateDisconnectedIgnition", func() {
		It("should generate disconnected ignition successfully", func() {
			...
			result, err := generator.GenerateDisconnectedIgnition(ctx, infraEnv, cluster)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should create correct directory structure", func() {
			...
			_, err := generator.GenerateDisconnectedIgnition(ctx, infraEnv, cluster)
			Expect(err).NotTo(HaveOccurred())
		})
		...
		It("should fail when cluster version is empty", func() {
			...
			_, err := generator.GenerateDisconnectedIgnition(ctx, infraEnv, emptyCluster)
			Expect(err).To(HaveOccurred())
		...
		It("should create agent-config.yaml with rendezvousIP from bootstrap host", func() {
			releaseImage := &models.ReleaseImage{
				CPUArchitecture:  swag.String(common.DefaultCPUArchitecture),
				OpenshiftVersion: swag.String("4.16.0"),
				URL:              swag.String("quay.io/openshift-release-dev/ocp-release:4.16.0-x86_64"),
			...
			mockExecuter.EXPECT().Execute(gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any(), gomock.Any()).DoAndReturn(func(command string, args ...string) (string, string, int) {
				...
				By("Verifying agent-config.yaml was created with rendezvousIP")
				agentConfigContent, err := os.ReadFile(filepath.Join(oveDir, "cluster-manifests", "agent-config.yaml"))
				Expect(err).NotTo(HaveOccurred())
				...
				Expect(agentConfig.APIVersion).To(Equal("v1alpha1"))
				Expect(agentConfig.Kind).To(Equal("AgentConfig"))
				Expect(agentConfig.Metadata.Name).To(Equal("test-cluster"))
				Expect(agentConfig.RendezvousIP).To(Equal("192.168.1.10"))
