...
var _ = Describe("Bootstrap Ignition Update", func() {
	...
	BeforeEach(func() {
		...
		g := NewGenerator(workDir, installerCacheDir, cluster, "", "", "", "", mockS3Client, log, mockOperatorManager, mockProviderRegistry).(*installerGenerator)
...
-----END CERTIFICATE-----`
	...
	Describe("update ignitions", func() {
		It("with ca cert file", func() {
			g := NewGenerator(workDir, installerCacheDir, cluster, "", "", caCertPath, "", nil, log, mockOperatorManager, mockProviderRegistry).(*installerGenerator)
		...
		It("with no ca cert file", func() {
			g := NewGenerator(workDir, installerCacheDir, cluster, "", "", "", "", nil, log, mockOperatorManager, mockProviderRegistry).(*installerGenerator)
			err := g.updateIgnitions()
			Expect(err).NotTo(HaveOccurred())
