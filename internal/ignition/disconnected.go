...
func createAgentConfig(infraEnv *common.InfraEnv, clusterName string, disconnectedDir string) error {
	if infraEnv.RendezvousIP == nil || string(*infraEnv.RendezvousIP) == "" {
		return nil
	}
	...
	agentConfig := AgentConfig{
		...
		RendezvousIP: string(*infraEnv.RendezvousIP),
	}
	...
	if err = os.WriteFile(filepath.Join(disconnectedDir, "agent-config.yaml"), agentConfigYAML, 0600); err != nil {
