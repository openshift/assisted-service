...
var _ = Describe("V2UpdateCluster", func() {
	...
	Context("load_balancer", func() {
		...
		Context("should succeed", func() {
			It("when updating to user managed load balancer with common VIP", func() {
				...
				cluster := &common.Cluster{
					Cluster: models.Cluster{
						ID:               &clusterID,
						OpenshiftVersion: common.MimimumVersionForUserManagedLoadBalancerFeature,
						Platform: &models.Platform{
		...
		It("when updating to VIPs part of the machine network, while cluster is with user managed load balancer", func() {
			apiVIPs := []*models.APIVip{
				{IP: "192.168.127.10"},
			}
		...
		It("when updating to VIPs outside of the machine netowk, while cluster is with user managed load balancer", func() {
			apiVIPs := []*models.APIVip{
				{IP: "192.168.128.10"},
			}
	...
	Context("should fail", func() {
		...
		It("when updating to VIPs outside of the machine netowk, while cluster is with cluster managed load balancer", func() {
			cluster := &common.Cluster{
				Cluster: models.Cluster{
					ID:               &clusterID,
					OpenshiftVersion: common.MimimumVersionForUserManagedLoadBalancerFeature,
...
`,
	...
	Context("with load_balancer", func() {
		BeforeEach(func() {
			Expect(envconfig.Process("test", &cfg)).ShouldNot(HaveOccurred())
			db, dbName = common.PrepareTestDB()
			bm = createInventory(db, cfg)
		...
		Context("should succeed", func() {
			...
			It("when registering cluster with user managed load balancer and common VIP", func() {
				...
				reply := bm.V2RegisterCluster(ctx, installer.V2RegisterClusterParams{
					NewClusterParams: &models.ClusterCreateParams{
						OpenshiftVersion: swag.String(common.MimimumVersionForUserManagedLoadBalancerFeature),
						CPUArchitecture:  common.X86CPUArchitecture,
						Platform: &models.Platform{
							Type: models.PlatformTypeBaremetal.Pointer(),
						},
