...
func (b *bareMetalInventory) checkFileForDownload(ctx context.Context, clusterID, fileName string) error {
	...
	case constants.KubeconfigNoIngress:
		err = clusterPkg.CanDownloadKubeconfigNoIngress(cluster)
...
func (b *bareMetalInventory) V2DownloadClusterCredentials(ctx context.Context, params installer.V2DownloadClusterCredentialsParams) middleware.Responder {
	...
	if err != nil && fileName == constants.Kubeconfig {
		...
		if err != nil && b.Config.ClusterTLSCertOverrideDir != "" {
			...
			// Allow KubeConfigNoIngress to be retrieved prior to installation complete
			clusterPkg.KubeConfigNoIngressAllowedStatuses = append(clusterPkg.KubeConfigNoIngressAllowedStatuses,
