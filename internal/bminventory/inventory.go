...
func (b *bareMetalInventory) V2DownloadClusterCredentials(ctx context.Context, params installer.V2DownloadClusterCredentialsParams) middleware.Responder {
	...
	if err != nil && fileName == constants.Kubeconfig {
		...
		if err != nil {
			kubeconfig_before_install := false

			if b.Config.ClusterTLSCertOverrideDir != "" {
				kubeconfig_before_install = true
				// Since only ABI uses this env variable, if it is set and the tls directory is empty then the
			...
			respBody, contentLength, err = b.v2DownloadClusterFilesInternal(ctx, fileName, params.ClusterID.String(), kubeconfig_before_install)
		}
	}
