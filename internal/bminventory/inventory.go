...
//nolint:gocyclo // This function is complex but we allow it for the consistency of cherry-picks
...
func (b *bareMetalInventory) createAndUploadDay2NodeIgnition(ctx context.Context, cluster *common.Cluster, host *models.Host) error {
	log := logutil.FromContext(ctx, b.log)
	...
	fullIgnition, err := b.IgnitionBuilder.FormatSecondDayWorkerIgnitionFile(address, host.MachineConfigPoolName, host)
	if err != nil {
...
func (b *bareMetalInventory) InstallSingleDay2HostInternal(ctx context.Context, clusterId strfmt.UUID, infraEnvId strfmt.UUID, hostId strfmt.UUID) error {
	...
	err = b.createAndUploadDay2NodeIgnition(ctx, cluster, &h.Host)
	if err != nil {
...
func (b *bareMetalInventory) V2InstallHost(ctx context.Context, params installer.V2InstallHostParams) middleware.Responder {
	...
	err = b.createAndUploadDay2NodeIgnition(ctx, cluster, h)
	if err != nil {
		log.Errorf("Failed to upload ignition for host %s", h.RequestedHostname)
		return common.GenerateErrorResponder(err)
	}
...
func (b *bareMetalInventory) InstallHosts(ctx context.Context, params installer.InstallHostsParams) middleware.Responder {
	...
	for i := range cluster.Hosts {
		...
		err = b.createAndUploadDay2NodeIgnition(ctx, cluster, cluster.Hosts[i])
		if err != nil {
...
func (b *bareMetalInventory) V2RegisterHost(ctx context.Context, params installer.V2RegisterHostParams) middleware.Responder {
	...
	if err := b.crdUtils.CreateAgentCR(ctx, log, params.NewHostParams.HostID.String(), infraEnv, cluster); err != nil {
		log.WithError(err).Errorf("Fail to create Agent CR, deleting host. Namespace: %s, InfraEnv: %s, HostID: %s", infraEnv.KubeKeyNamespace, infraEnv.Name, params.NewHostParams.HostID.String())
