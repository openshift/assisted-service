...
var _ = Describe("Transition tests", func() {
	...
	Context("cancel_installation", func() {
		BeforeEach(func() {
			capi = NewManager(getDefaultConfig(), common.GetTestLog(), db, testing.GetDummyNotificationStream(ctrl), eventsHandler, uploadClient, nil, mockMetric, nil, nil, operatorsManager, nil, nil, nil, nil, nil)
		})
	...
	Context("complete_installation", func() {
		...
		for i := range tests {
			...
			It(t.name, func() {
				...
				capi = NewManager(getDefaultConfig(), common.GetTestLog(), db, testing.GetDummyNotificationStream(ctrl), eventsHandler, uploadClient, nil, mockMetric, nil, nil, operatorsManager, ocmClient, mockS3Api, nil, nil, nil)
...
var _ = Describe("Cancel cluster installation", func() {
	...
	BeforeEach(func() {
		...
		uploadClient = uploader.NewClient(&uploader.Config{EnableDataCollection: false}, nil, logrus.New(), nil)
		capi = NewManager(getDefaultConfig(), common.GetTestLog(), db, testing.GetDummyNotificationStream(ctrl), mockEventsHandler, uploadClient, nil, mockMetric, nil, nil, operatorsManager, nil, nil, nil, nil, nil)
	})
...
var _ = Describe("Reset cluster", func() {
	...
	BeforeEach(func() {
		...
		mockEventsHandler = eventsapi.NewMockHandler(ctrl)
		operatorsManager := operators.NewManager(common.GetTestLog(), nil, operators.Options{}, nil, nil)
		capi = NewManager(getDefaultConfig(), common.GetTestLog(), db, testing.GetDummyNotificationStream(ctrl), mockEventsHandler, nil, nil, nil, nil, nil, operatorsManager, nil, nil, nil, nil, nil)
	})
...
var _ = Describe("Refresh Cluster - No DHCP", func() {
	...
	BeforeEach(func() {
		...
		clusterApi = NewManager(getDefaultConfig(), common.GetTestLog().WithField("pkg", "cluster-monitor"), db, testing.GetDummyNotificationStream(ctrl),
			mockEvents, nil, mockHostAPI, mockMetric, nil, nil, operatorsManager, nil, mockS3Api, nil, nil, nil)
...
var _ = Describe("RefreshCluster - preparing for install", func() {
	...
	BeforeEach(func() {
		...
		clusterApi = NewManager(getDefaultConfig(), common.GetTestLog().WithField("pkg", "cluster-monitor"), db, testing.GetDummyNotificationStream(ctrl),
			mockEvents, nil, mockHostAPI, mockMetric, nil, nil, operatorsManager, nil, nil, dnsApi, nil, nil)
		...
		tests := []struct {
			...
			},
			{
				...
				validationsChecker: makeJsonChecker(map[ValidationID]validationCheckResult{
					...
					IsMachineCidrEqualsToCalculatedCidr: {status: ValidationSuccess, messagePattern: "Cluster Machine CIDR is equivalent to the calculated CIDR"},
					AreApiVipsDefined:                   {status: ValidationSuccess, messagePattern: "API virtual IPs are defined"},
					AreApiVipsValid:                     {status: ValidationFailure, messagePattern: fmt.Sprintf("api vips <%s> is already in use in cidr %s", common.TestIPv4Networking.APIVip, string(common.TestIPv4Networking.MachineNetworks[0].Cidr))},
					AreIngressVipsDefined:               {status: ValidationSuccess, messagePattern: "Ingress virtual IPs are defined"},
					AreIngressVipsValid:                 {status: ValidationFailure, messagePattern: fmt.Sprintf("ingress vips <%s> is already in use in cidr %s", common.TestIPv4Networking.IngressVip, string(common.TestIPv4Networking.MachineNetworks[0].Cidr))},
					AllHostsAreReadyToInstall:           {status: ValidationSuccess, messagePattern: "All hosts in the cluster are ready to install."},
					IsDNSDomainDefined:                  {status: ValidationSuccess, messagePattern: "The base domain is defined"},
					IsPullSecretSet:                     {status: ValidationSuccess, messagePattern: "The pull secret is set."},
					SufficientMastersCount:              {status: ValidationSuccess, messagePattern: "The cluster has the exact amount of dedicated control plane nodes."},
		...
		for i := range tests {
			...
			It(t.name, func() {
				...
				if t.withOCMClient {
					...
					ocmClient := &ocm.Client{AccountsMgmt: mockAccountsMgmt, Config: &ocm.Config{}}
					clusterApi = NewManager(getDefaultConfig(), common.GetTestLog().WithField("pkg", "cluster-monitor"), db, testing.GetDummyNotificationStream(ctrl),
						mockEvents, nil, mockHostAPI, mockMetric, nil, nil, operatorsManager, ocmClient, mockS3Api, nil, nil, nil)
