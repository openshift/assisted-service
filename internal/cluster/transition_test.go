		...
		tests := []struct {
			...
			},
			{
				...
				validationsChecker: makeJsonChecker(map[ValidationID]validationCheckResult{
					...
					IsPullSecretSet:                     {status: ValidationSuccess, messagePattern: "The pull secret is set"},
					isNetworkTypeValid:                  {status: ValidationSuccess, messagePattern: "The cluster has a valid network type"},
					SufficientMastersCount:              {status: ValidationFailure, messagePattern: "The cluster must have exactly 2 dedicated control plane nodes and at least 1 arbiter node. Add or remove hosts, or change their roles configurations to meet the requirement."},
				}),
				errorExpected:     false,
				openshiftVersion:  common.MinimumVersionForArbiterClusters,
				controlPlaneCount: 2,
