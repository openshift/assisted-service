	...
	for {
		...
		for _, cluster := range clusters {
			...
			if errors.Is(ctxCluster.Err(), context.DeadlineExceeded) {
				...
				m.eventsHandler.NotifyInternalEvent(cycle.ctx, cluster.ID, nil, nil, fmt.Sprintf("cluster monitor deadline exceeded; blacklisting cluster %s for %s", cluster.ID.String(), m.Config.MonitorBlacklistDuration))
				continue
			}
			if err != nil {
