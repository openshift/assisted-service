...
var _ = Describe("TestClusterMonitoring", func() {
	...
	BeforeEach(func() {
		...
		mockOperators.EXPECT().ValidateCluster(gomock.Any(), gomock.Any()).AnyTimes().Return([]api.ValidationResult{
			...
			{Status: api.Success, ValidationId: string(models.ClusterValidationIDNodeMaintenanceRequirementsSatisfied)},
			{Status: api.Success, ValidationId: string(models.ClusterValidationIDKubeDeschedulerRequirementsSatisfied)},
			{Status: api.Success, ValidationId: string(models.ClusterValidationIDClusterObservabilityRequirementsSatisfied)},
...
var _ = Describe("insufficient_state", func() {
	...
	It("works", func() {
		replyErr := clusterApi.RegisterCluster(ctx, &cluster, models.ClusterStatusInsufficient)
...
var _ = Describe("Update AMS subscription ID", func() {
	...
	It("Update AMS subscription ID", func() {
		...
		err := api.RegisterCluster(ctx, &c, models.ClusterStatusInsufficient)
		Expect(err).ShouldNot(HaveOccurred())
...
var _ = Describe("Validation metrics and events", func() {
	...
	registerTestClusterWithValidationsAndHost := func() *common.Cluster {
		...
		err = m.RegisterCluster(ctx, &c, models.ClusterStatusInsufficient)
		Expect(err).ShouldNot(HaveOccurred())
