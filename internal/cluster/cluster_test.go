...
var _ = Describe("Permanently delete clusters", func() {
	...
	It("permanently delete clusters success", func() {
		...
		dbReply := db.Unscoped().Model(&common.Cluster{}).Where("id = ?", c1.ID).Update("deleted_at", deletedAt)
		Expect(dbReply.RowsAffected).Should(Equal(int64(1)))
		dbReply = db.Unscoped().Model(&common.Cluster{}).Where("id = ?", c2.ID).Update("deleted_at", deletedAt)
		Expect(dbReply.RowsAffected).Should(Equal(int64(1)))

		Expect(state.PermanentClustersDeletion(ctx, deletedAt, mockS3Api)).ShouldNot(HaveOccurred())

		Expect(db.Where("id = ?", c1.ID).Find(&common.Cluster{}).RowsAffected).Should(Equal(int64(0)))
		Expect(db.Where("id = ?", c2.ID).Find(&common.Cluster{}).RowsAffected).Should(Equal(int64(0)))
