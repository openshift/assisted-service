...
func ValidateClusterCreateIPAddresses(ipV6Supported bool, clusterId strfmt.UUID, params *models.ClusterCreateParams) error {
	...
	if params.Platform != nil && params.Platform.Type != nil {
		targetConfiguration.Platform = params.Platform
	} else {
		targetConfiguration.Platform = &models.Platform{Type: common.PlatformTypePtr(models.PlatformTypeBaremetal)}
	}
	...
	targetConfiguration.UserManagedNetworking = nil // TODO - remove when removing cluster property
	targetConfiguration.VipDhcpAllocation = params.VipDhcpAllocation
...
func validateVIPsWithUMN(cluster *common.Cluster, params *models.V2ClusterUpdateParams, vipDhcpAllocation bool) error {
	var (
		apiVip      string
		ingressVip  string
	...
	if swag.BoolValue(cluster.VipDhcpAllocation) {
		return ValidateVIPsWereNotSetUserManagedNetworking(
			platform, apiVip, ingressVip, apiVips, ingressVips, vipDhcpAllocation,
		)
	}
...
func ValidateClusterUpdateVIPAddresses(ipV6Supported bool, cluster *common.Cluster, params *models.V2ClusterUpdateParams) error {
	...
	if featuresupport.IsActivePlatformSupportsUmn(cluster, params) {
		...
		if !featuresupport.IsActivePlatformSupportsCmn(cluster, params) {
			if err = validateVIPsWithUMN(cluster, params, vipDhcpAllocation); err != nil {
	...
	targetConfiguration.Platform = cluster.Platform
	if params.Platform != nil && params.Platform.Type != nil {
		targetConfiguration.Platform = params.Platform
	}
...
func validateVIPAddresses(ipV6Supported bool, targetConfiguration common.Cluster) error {
	...
	if common.IsClusterUmnEnabled(&targetConfiguration) {
		if err = ValidateVIPsWereNotSetUserManagedNetworking(targetConfiguration.Platform, targetConfiguration.APIVip, targetConfiguration.IngressVip,
...
func ValidateVIPsWereNotSetUserManagedNetworking(platform *models.Platform, apiVip string, ingressVip string, apiVips []*models.APIVip, ingressVips []*models.IngressVip, vipDhcpAllocation bool) error {
	if vipDhcpAllocation {
		err := errors.Errorf("VIP DHCP Allocation cannot be set with %s platform", *platform.Type)
		return err
	}
	...
	if ingressVip != "" || len(ingressVips) > 0 {
		if len(errMessage) == 0 {
			errMessage = "Ingress VIP"
		} else {
			errMessage += " and Ingress VIP"
		}
	}
	if errMessage != "" {
		err := errors.Errorf("%s cannot be set with %s platform", errMessage, *platform.Type)
		return err
	}
