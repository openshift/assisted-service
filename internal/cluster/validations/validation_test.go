...
var _ = Describe("VIP Dual-Stack Validation", func() {
	...
	Describe("validateVIPAddressFamily - Dual Stack API VIPs", func() {
		Context("OCP 4.13+ (IPv6-primary allowed)", func() {
		...
		Context("OCP 4.12 and below (IPv4-primary only)", func() {
			BeforeEach(func() {
				cluster.OpenshiftVersion = "4.12.0"
			})
			...
			It("rejects IPv6-first API VIPs", func() {
				...
				Expect(err.Error()).To(ContainSubstring("IPv6-primary dual-stack requires OpenShift 4.13+"))
			})
...
var _ = Describe("Machine Network amount and order", func() {
	tests := []struct {
		...
		},
		{
			...
			valid:   false,
			description: "IPv6-primary dual-stack (not supported in 4.12)",
			openshiftVersion: "4.12.0",
		},
		{
			element: []*models.MachineNetwork{{Cidr: "1002:db8::/119"}, {Cidr: "1.2.5.0/24"}},
