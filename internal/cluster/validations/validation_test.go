...
const (
	...
	validSecretFormatUpdatedIPV6Disconnected = "{\"auths\":{\"cloud.openshift.com\":{\"auth\":\"dXNlcjpwYXNzd29yZAo=\",\"email\":\"r@r.com\"},\"quay.io\":{\"auth\":\"dXNlcjpwYXNzd29yZAo=\",\"email\":\"r@r.com\"},\"registry.connect.redhat.com\":{\"auth\":\"dXNlcjpwYXNzd29yZAo=\",\"email\":\"r@r.com\"},\"registry.stage.redhat.io\":{\"auth\":\"c29tZW9uZUBleGFtcGxlLmNvbTp0aGlzaXNhc2VjcmV0\"}}}"
...
var _ = Describe("Pull secret validation", func() {
	...
	Context("test secret format", func() {
		...
		It("valid format", func() {
			err := secretValidator.ValidatePullSecret(validSecretFormat, "", authHandlerDisabled, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
		It("empty secret", func() {
			err := secretValidator.ValidatePullSecret("", "", authHandlerDisabled, true)
			Expect(err).Should(HaveOccurred())
		})
		It("invalid format for the auth", func() {
			err := secretValidator.ValidatePullSecret(invalidAuthFormat, "", authHandlerDisabled, true)
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(BeAssignableToTypeOf(&PullSecretError{}))
		})
		It("invalid format", func() {
			err := secretValidator.ValidatePullSecret(invalidSecretFormat, "", authHandlerDisabled, true)
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(BeAssignableToTypeOf(&PullSecretError{}))
		})
		It("valid format - Invalid user", func() {
			err := secretValidator.ValidatePullSecret(validSecretFormat, "NotSameUser@example.com", authHandler, true)
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(BeAssignableToTypeOf(&PullSecretError{}))
		})
		It("valid format - Valid user", func() {
			err := secretValidator.ValidatePullSecret(validSecretFormat, userName, authHandler, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
	...
	Context("test registries", func() {
		...
		It("pull secret accepted when it contains all required registries", func() {
			...
			Expect(err).ShouldNot(HaveOccurred())
			err = validator.ValidatePullSecret(validSecretFormat, "", authHandlerDisabled, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
		...
		It("docker.io auth is accepted when there is an image from docker.io", func() {
			...
			Expect(err).ShouldNot(HaveOccurred())
			err = validator.ValidatePullSecret(pullSecDocker, "", authHandlerDisabled, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
		...
		It("legacy DockerHub auth is accepted when there is an image from docker.io", func() {
			validator, err := NewPullSecretValidator(Config{}, "docker.io/testing:latest")
			Expect(err).ShouldNot(HaveOccurred())
			err = validator.ValidatePullSecret(pullSecLegacyDocker, "", authHandlerDisabled, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
		...
		It("error when pull secret does not contain required registry", func() {
			...
			Expect(err).ShouldNot(HaveOccurred())
			err = validator.ValidatePullSecret(validSecretFormat, "", authHandlerDisabled, false)
			Expect(err).Should(HaveOccurred())
			Expect(err).Should(BeAssignableToTypeOf(&PullSecretError{}))
		})
		It("Skip registry.redhat.io auth validation in case of IPV6 disconnected cluster", func() {
			validator, err := NewPullSecretValidator(Config{}, "quay.io/testing:latest", "registry.redhat.io/image:v1")
			Expect(err).ShouldNot(HaveOccurred())
			err = validator.ValidatePullSecret(validSecretFormatUpdatedIPV6Disconnected, "", authHandlerDisabled, true)
			Expect(err).ShouldNot(HaveOccurred())
		})
	})
})
