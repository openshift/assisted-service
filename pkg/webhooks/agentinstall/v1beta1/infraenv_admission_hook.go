...
func (a *InfraEnvValidatingAdmissionHook) Initialize(kubeClientConfig *rest.Config, stopCh <-chan struct{}) error {
	...
	if err := hiveext.AddToScheme(scheme); err != nil {
		return fmt.Errorf("failed to add hiveext scheme: %w", err)
	}
...
func (a *InfraEnvValidatingAdmissionHook) osImageVersionValid(logger *log.Entry, oldObject *v1beta1.InfraEnv, newObject *v1beta1.InfraEnv) bool {
	...
	if clusterDeployment.Spec.ClusterInstallRef == nil {
		logger.Error("Cluster install ref is not set")

		return false
	}

	if clusterDeployment.Spec.ClusterInstallRef.Kind != "AgentClusterInstall" {
		logger.Error("Cluster install ref is not an AgentClusterInstall")

		return true // Weird case, it should not happen, but if it does we shouldn't block the request
	}
