...
func (a *AgentValidatingAdmissionHook) validateCreate(admissionSpec *admissionv1.AdmissionRequest) *admissionv1.AdmissionResponse {
	...
	// Ensure that InstallationDiskID and InstallationDiskByPath are not both set
	if newObject.Spec.InstallationDiskID != "" && newObject.Spec.InstallationDiskByPath != "" {
		message := "Either InstallationDiskID or InstallationDiskByPath can be set, not both."
