...
func getControlPlaneCount(originalObject, updatesObject *hiveext.AgentClusterInstall) int64 {
	...
	if controlPlaneAgents == 1 && workerAgents == 0 { // SNO
		return int64(1)
	} else if controlPlaneAgents >= common.MinMasterHostsNeededForInstallationInHaMode && controlPlaneAgents <= common.MaxMasterHostsNeededForInstallationInHaModeOfOCP418OrNewer {
		return int64(controlPlaneAgents)
	}
	return int64(common.MinMasterHostsNeededForInstallationInHaMode)
}
