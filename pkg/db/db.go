...
// client_encoding=UTF8 is always set so pgx v5 simple-query paths (used by some GORM APIs)
// work regardless of server defaults.
// The resulting string is validated with pgx ParseConfig to avoid accepting malformed input.
func LibpqDSN(host, port, user, password, database string) (string, error) {
	...
	if _, err := pgconn.ParseConfig(s); err != nil {
		return "", err
	}
	return s, nil
}
...
func (c Config) LibpqDSN() (string, error) {
