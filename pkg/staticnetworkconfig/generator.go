...
type Nmstate interface {
	GenerateConfiguration(state string) (string, error)
}
...
func New(log logrus.FieldLogger, nm *nmstate.Nmstate) StaticNetworkConfig {
	if nm == nil {
		nm = nmstate.New()
	}
...
func (s *StaticNetworkConfigGenerator) generateHostStaticNetworkConfigData(hostConfig *models.HostStaticNetworkConfig, hostDir string) ([]StaticNetworkConfigData, error) {
	...
	result, err := s.validateNMStateConfig(hostYAML)
	if err != nil {
		return nil, err
	}
...
func (s *StaticNetworkConfigGenerator) validateNMStateConfig(hostYAML string) (string, error) {
	...
	if err != nil {
		...
		return "", fmt.Errorf("failed to execute 'nmstatectl gc', error: %s", err.Error())
	}
	return stdout, nil
}
...
func (s *StaticNetworkConfigGenerator) validateNMStateYaml(networkYaml string) error {
	result, err := s.validateNMStateConfig(networkYaml)
	if err != nil {
		return err
	}
