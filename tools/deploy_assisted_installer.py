...
DST_FILE = os.path.join(
    os.getcwd(), "build", deploy_options.namespace, "assisted-service.yaml"
)
KEY_FILE = os.path.join(
    os.getcwd(), "build", deploy_options.namespace, "auth-test-pub.json"
)
...
def load_key():
    try:
        ...
        log.warn(
            "Got exception {}, when tried to read key file at {}."
            "Make sure you used tools/auth_keys_generator.go before running subsystem tests".format(
                e, KEY_FILE
            )
        )
        return ""
...
def main():
    ...
    with open(SRC_FILE) as src:
        ...
        if deploy_options.subsystem_test:
            if (
                data["spec"]["template"]["spec"]["containers"][0].get("env", None)
                is None
            ):
            ...
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {
                    "name": "DEREGISTER_WORKER_INTERVAL",
                    "value": TEST_DEREGISTER_WORKER_INTERVAL,
                }
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {
                    "name": "DELETION_WORKER_INTERVAL",
                    "value": TEST_DELETION_WORKER_INTERVAL,
                }
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {
                    "name": "INFRAENV_DELETION_WORKER_INTERVAL",
                    "value": TEST_INFRAENV_DELETION_WORKER_INTERVAL,
                }
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "JWKS_CERT", "value": load_key()}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "SUBSYSTEM_RUN", "value": "True"}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "DUMMY_IGNITION", "value": "True"}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "OCM_BASE_URL", "value": WIREMOCK_SERVICE}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "OCM_TOKEN_URL", "value": WIREMOCK_SERVICE + "/token"}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "OCM_SERVICE_CLIENT_ID", "value": "mock-ocm-client-id"}
            )
            data["spec"]["template"]["spec"]["containers"][0]["env"].append(
                {"name": "OCM_SERVICE_CLIENT_SECRET", "value": "mock-ocm-client-secret"}
            )
        ...
        if deploy_options.image_pull_policy:
            data["spec"]["template"]["spec"]["containers"][0][
                "imagePullPolicy"
            ] = deploy_options.image_pull_policy
