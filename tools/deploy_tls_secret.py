...
def get_ca(output_dir, force_replace=False):
    ...
    if force_replace or not os.path.exists(ca_csr_path):
        print(
            utils.check_output(
                f'openssl req -x509 -nodes -subj "{ca_subject}" -days {ca_expiration} '
...
def generate_secret(
    output_dir, service, san, namespace, expiration=120, keep_files=False
):
    ...
    print(
        utils.check_output(
            f'openssl req -new -newkey rsa:2048 -nodes -subj "/CN={service}" '
    ...
    print(
        utils.check_output(
            f"openssl x509 -req -days {expiration} "
            f'-extfile "{ext_file}" '
