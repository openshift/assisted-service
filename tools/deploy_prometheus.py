...
def deploy_oauth_reqs():
    ...
    if not utils.check_if_exists(
        ...
        cmd = (
            "{} -n {} create secret generic {} --from-literal=session_secret={}".format(
                CMD_BIN, deploy_options.namespace, secret_name, session_secret
            )
        )
    ...
    if not cert_secret:
        ...
        cmd = "{} get secret {} --namespace={} --template '{}'".format(
            CMD_BIN, secret_name, namespace, template
        )
        ...
        dst_file = os.path.join(
            ...
            "assisted-installer-ocp-prometheus-custom-ca.yaml",
        )
        topic = "OCP Custom CA"
...
def deploy_prometheus_route():
    ...
    dst_file = os.path.join(
        ...
        "build",
        deploy_options.namespace,
        "assisted-installer-ocp-prometheus-route.yaml",
    )
    try:
        # I have permissions
...
def deploy_prometheus_sub(olm_ns, cat_src):
    ...
    dst_file = os.path.join(
        ...
        "build",
        deploy_options.namespace,
        "assisted-installer-operator-subscription.yaml",
    )
    with open(src_file) as src:
        with open(dst_file, "w+") as dst:
            data = src.read()
    ...
    if deploy_options.target != "oc-ingress":
        ...
        try:
            ...
            if not utils.check_output(cmd):
                print(
                    "The creation of an OperatorGroup is Forbidden for you user please request a creation of one before execute this again, exiting..."
                )
                sys.exit(1)
            else:
