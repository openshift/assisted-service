...
def load_deployment_options(parser=None):
    ...
    parser.add_argument(
        ...
        choices=[
            MINIKUBE_TARGET,
            OPENSHIFT_TARGET,
            INGRESS_REMOTE_TARGET,
            OCP_TARGET,
            KIND_TARGET,
        ],
        default=MINIKUBE_TARGET,
    )
    parser.add_argument("--domain", help="Target domain", type=str)
    ...
    parser.add_argument(
        "--storage", help="Assisted service storage", type=str, default="s3"
    )

    parser.add_argument(
        "--apply-manifest", type=lambda x: (str(x).lower() == "true"), default=True
    )
    parser.add_argument(
        "--persistent-storage", type=lambda x: (str(x).lower() == "true"), default=True
    )
    parser.add_argument(
        "-p",
        "--port",
    ...
    parser.add_argument(
        "--image-pull-policy",
        help="Determine if the image should be pulled prior to starting the container.",
        type=str,
    ...
    deploy_options.add_argument(
        "--disable-tls",
        action="store_true",
        help="Disable TLS for assisted service transport",
        default=False,
    )
...
def get_manifest_from_url(tag):
    manifest_file = get_file_content(
        "https://api.github.com/repos/openshift-assisted/assisted-installer-deployment",
        tag,
...
def get_image_override(
    deployment_options, short_image_name, env_var_name, org="edge-infrastructure"
):
    ...
    if deployment_options.deploy_manifest_path:
        ...
        with open(deployment_options.deploy_manifest_path) as manifest_contnet:
