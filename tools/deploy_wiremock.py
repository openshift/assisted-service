...
def deploy_wiremock(deploy_options: Namespace):
    ...
    if deploy_options.target == "kind":
        utils.override_service_type_definition_and_node_port(
            internal_definitions_path="build/wiremock-deployment.yaml",
            internal_target_definitions_path="build/wiremock-deployment.yaml",
    ...
    if deploy_options.target == "kind":
        hostname = os.environ.get('IP', 'localhost')
        port="8070"

    # Ensure the Wiremock pod is ready before populating the stubs.
    # To verify if Wiremock is ready to receive requests,
    # we need to obtain its hostname and port. In environments other than KIND,
    # these should be fetched from the service's external IP and port.
    else:
