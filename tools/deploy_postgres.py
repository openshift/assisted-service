...
def deploy_postgres_secret(deploy_options):
    ...
    dst_file = utils.dump_yaml_file_docs(
        basename=f"build/{deploy_options.namespace}/postgres-secret.yaml", docs=docs
    )
...
def deploy_postgres(deploy_options):
    ...
    dst_file = utils.dump_yaml_file_docs(
        basename=f"build/{deploy_options.namespace}/postgres-deployment.yaml", docs=docs
    )
...
def deploy_postgres_storage(deploy_options):
    ...
    dst_file = utils.dump_yaml_file_docs(
        basename=f"build/{deploy_options.namespace}/postgres-storage.yaml", docs=docs
    )
