...
def deploy_oauth_reqs():
    ...
    if not utils.check_if_exists(
        ...
        cmd = (
            f"{CMD_BIN} -n {deploy_options.namespace} create serviceaccount {sa_name} "
        )
        utils.check_output(cmd)
...
def deploy_grafana_route():
    ...
    dst_file = os.path.join(
        ...
        "build",
        deploy_options.namespace,
        "assisted-installer-ocp-grafana-route.yaml",
    )
    try:
        # I have permissions
...
def deploy_grafana_ds():
    ...
    dst_file = os.path.join(
        os.getcwd(), "build", deploy_options.namespace, "prometheus.json"
    )
    with open(src_file) as fp:
        data = fp.read()
    data = data.replace("REPLACE_NAMESPACE", f'"{deploy_options.namespace}"')
