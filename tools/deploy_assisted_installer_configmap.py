...
def handle_arguments():
    ...
    parser.add_argument(
        "--jwks-url", default="https://api.openshift.com/.well-known/jwks.json"
    )
    parser.add_argument(
        "--ocm-url",
        default="https://api-integration.6943.hive-integration.openshiftapps.com",
    )
...
DST_FILE = os.path.join(
    os.getcwd(), "build", deploy_options.namespace, "assisted-service-configmap.yaml"
)
...
def main():
    ...
    verify_images(
        release_images=json.loads(json.loads(f'"{deploy_options.release_images}"'))
    )
    ...
    data = data.replace(
        "REPLACE_CHECK_CLUSTER_VERSION_FLAG", f'"{deploy_options.check_cvo}"'
    )
    ...
    data = data.replace(
        "REPLACE_OPENSHIFT_VERSIONS", f'"{deploy_options.ocp_versions}"'
    )
    data = data.replace("REPLACE_OS_IMAGES", f'"{deploy_options.os_images}"')
    data = data.replace("REPLACE_RELEASE_IMAGES", f'"{deploy_options.release_images}"')
    data = data.replace(
        "REPLACE_MUST_GATHER_IMAGES", f'"{deploy_options.must_gather_images}"'
    )
    data = data.replace(
        "REPLACE_PUBLIC_CONTAINER_REGISTRIES", f'"{deploy_options.public_registries}"'
    )
    data = data.replace("REPLACE_IPV6_SUPPORT", f'"{deploy_options.ipv6_support}"')
    data = data.replace("REPLACE_ISO_IMAGE_TYPE", f'"{deploy_options.iso_image_type}"')
    data = data.replace(
        "REPLACE_HW_VALIDATOR_REQUIREMENTS", f'"{deploy_options.hw_requirements}"'
    )
    data = data.replace(
        "REPLACE_DISABLED_HOST_VALIDATIONS",
        f'"{deploy_options.disabled_host_validations}"',
    )
    data = data.replace("REPLACE_DISABLED_STEPS", f'"{deploy_options.disabled_steps}"')
    ...
    y["data"][
        "ENABLE_ORG_BASED_FEATURE_GATES"
    ] = deploy_options.enable_org_based_feature_gates
