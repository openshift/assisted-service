...
HOST_WEIGHT_GPU_WEIGHT = os.environ.get("HOST_WEIGHT_GPU_WEIGHT", "10")
...
def main():
    ...
    if deploy_options.apply_manifest:
        if deploy_options.target == "kind":
            assisted_service_base_url = f"http://{os.environ.get('IP', 'localhost')}:8090"
    ...
    data = data.replace('HOST_WEIGHT_GPU_WEIGHT', "'{}'".format(HOST_WEIGHT_GPU_WEIGHT))
