...
function generate_test_versions() {
    local os_images_file="${__root}/data/default_os_images.json"
    local release_images_file="${__root}/data/default_release_images.json"
    local output_file="${__root}/internal/common/test_versions_generated.go"
    local cluster_image_set_file="${__root}/docs/hive-integration/crds/clusterImageSet.yaml"

    python3 -c '
import json
from collections import defaultdict

with open("'"${os_images_file}"'") as f:
    os_data = json.load(f)

with open("'"${release_images_file}"'") as f:
    release_data = json.load(f)

os_images = defaultdict(dict)
for entry in os_data:
    arch = entry["cpu_architecture"]
    parts = entry["openshift_version"].split(".")
    version = "%s.%s" % (parts[0], parts[1])
    os_images[arch][version] = {"version": entry["version"], "url": entry["url"]}

release_images = defaultdict(dict)
for entry in release_data:
    arch = entry["cpu_architecture"]
    ov = entry["openshift_version"].replace("-multi", "")
    parts = ov.split(".")
    version = "%s.%s" % (parts[0], parts[1])
    release_images[arch][version] = {"version": entry["version"], "url": entry["url"]}

def sort_key(v):
    return tuple(int(p) for p in v.split(".") if p.isdigit())

def format_image_map(images):
    lines = []
    for arch in sorted(images):
        lines.append("\t\"%s\": {" % arch)
        for v in sorted(images[arch], key=sort_key):
            img = images[arch][v]
            lines.append("\t\t\"%s\": {\n\t\t\tVersion: \"%s\",\n\t\t\tURL:     \"%s\",\n\t\t}," % (v, img["version"], img["url"]))
        lines.append("\t},")
    return "\n".join(lines)

with open("'"${output_file}"'", "w") as f:
    f.write("""// Code generated by hack/generate.sh generate_test_versions; DO NOT EDIT.
package common
...
%s
}""" % (format_image_map(os_images), format_image_map(release_images)))

# --- Update clusterImageSet.yaml ---

import re, sys

x86_os_versions = sorted(os_images.get("x86_64", {}).keys(), key=sort_key)
if not x86_os_versions:
    print("No x86_64 OS image versions found", file=sys.stderr)
    sys.exit(1)

newest = x86_os_versions[-1]
x86_release = release_images.get("x86_64", {}).get(newest)
if not x86_release:
    print("No x86_64 release image found for version %s" % newest, file=sys.stderr)
    sys.exit(1)

with open("'"${cluster_image_set_file}"'") as f:
    content = f.read()

content = re.sub(r"(name: openshift-v)\S+", r"\g<1>" + x86_release["version"].split("-")[0], content)
content = re.sub(r"(releaseImage: )\S+", r"\g<1>" + x86_release["url"], content)

with open("'"${cluster_image_set_file}"'", "w") as f:
    f.write(content)
'
