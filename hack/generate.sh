...
function generate_python_client() {
    rm -rf "${__root}"/assisted-service-client*
    ...
    docker run --rm -u "$(id -u)" --entrypoint /bin/sh \
     ...
     cp "${__root}"/tools/client_package_initializer.py "${__root}"/assisted-service-client/client_package_initializer.py
     cd "${__root}"/assisted-service-client/ && python3 setup.py sdist --dist-dir "${__root}"
     cd "${__root}"/assisted-service-client/ && python3 client_package_initializer.py ./  https://github.com/openshift/assisted-service --build
}
